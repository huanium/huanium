...
I would like to begin by thanking Professor Evan Randles for his guidance, especially throughout the past year and a half. His infectious enthusiasm for mathematics (and physics!) inspired my eventual journey through the mathematics major at Colby. His incredible patience and trust gave me the courage to take on, explore, and contribute to the research project. His attention to detail set the bar high and made me a better student of mathematics. I will always treasure the thousands of hours of brainstorming, writing, and re-writing on Overleaf and every single discussion we had, be it a 15-minute check-in or a multi-hour thinking session, early or late, in person or over Zoom, with or without the company of Eleanor and David. Needless to say, this work would not have been possible without Professor Randles.\\


Next, I would like to thank Professor Leo Livshits not only for agreeing to be the second reader for this paper and providing many insightful suggestions, but also for his MA353: Matrix Analysis (now Advanced Linear Algebra). MA353 completely changed the way I think about pure mathematics and gave me a powerful toolbox for
exploring more advanced mathematics and physics.  \\


I would also like to thank Professors Tamar Friedmann, Fernando Gouv\^{e}a, Benjamin Mathes, Otto Bretscher, Liam O'Brien, Jerzy Wieczorek for enriching my mathematical education in wonderful ways, Colby Department of Physics and Astronomy for sparking my interest in pure mathematics, and Professor Laurent Saloff-Coste for the helpful suggestions when this project was still in its formative stage.  \\



Finally, I would like to thank my friends and family. To my life-long friend Hung Duong: our intercontinental classical guitar sessions and random conversations never fail to brighten up my weekends. To my dear friends James Ishiguro,  Allen Ma, Matt Welch, and Colby Ultimate Frisbee: you guys made Colby my home. To Mom, Dad, and Sister: your unconditional love and support are the only reason I am here today. To you I dedicate this thesis.
...
In this chapter, we make precise and extend the definition of positive homogeneous polynomials as it appeared in \cite{randles_convolution_2017}. We also introduce the notions of \textbf{subhomogeneous functions} and \textbf{strongly subhomogeneous functions}, which appear alongside positive homogeneous polynomials/functions in a certain Taylor expansion related to the Fourier transform of $\phi$. These classes of functions are key ingredients for estimating the convolution powers of $\phi$ via the Van der Corput lemma, particularly in the cases where oscillatory integrals are involved. This chapter is essentially Section 2 of \cite{bui2021generalized}.\\

\noindent 
To start, we shall review the notion of continuous one-parameter groups defined in Definition \ref{def:cont_one_param_group}. Continuous one-parameter groups appear naturally when one characterizes the radial scaling when parameterizing $\mathbb{R}^d$. For instance, in the usual polar coordinates, we have \textbf{isotropic} scaling in the radial variable $r$ and can simply choose our continuous one-parameter group to be one which corresponds nicely with the Euclidean norm: $\{T_r \}_{r>0} = r^E$ where $E=\diag(1/2, 1/2)$ in the standard representation. It is clear that $\{T_r \}_{r>0}$ is a group, and that any point $v\in \mathbb{R}^d \setminus \{0\}$ can be written uniquely as $r^E \eta$ where $\eta\in \mathbb{S}$. In general, $E$ needs not be a multiple of the identity transformation $\mathbb{I}$ (whose standard representation is of course $\diag(1,1)$). In such cases, we have \textbf{anisotropic} scaling. For example, let $E'$ be $\diag(1/2, 1/4)$ in the standard basis. Any point $v\in \mathbb{R}^2\setminus\{0\}$ can also be written uniquely as $r^{E'}\eta'$ where $\eta' \in \{ (\eta'_1,\eta'_2) \in \mathbb{R}^2 : {\eta'}_1^{2} + {\eta'}_2^{4} = 1  \}$ and $r> 0$. 
...
\begin{enumerate}
    ...
    \item $T_r$ is continuous in $r$ (in the usual topology of $\Gl({\mathbb{R}^d})$);    
    \item $T_0 = \mathbb{I}_d$, where $\mathbb{I}_d$ denotes the identity map on $\mathbb{R}^d$;
...
\end{equation*}
for some $E\in\End(\mathbb{R}^d)$. $E$ is called the (infinitesimal) generator of $\{T_r\}$, and $\{T_r\}$ is said to be generated by $E$. For simplicity, we shall use this representation throughout the paper, i.e., any element of $\{T_r \}_{r>0}$ has the form $r^E$. We write $T_r= r^E$ to be explicit about $E$.\\

\noindent An important possible feature of a continuous one-parameter group $\{T_r \}_{r>0}$ is the \textbf{contracting} property. Intuitively, this means that the radial scaling becomes vanishingly small as the parameter $r$ approaches $0$. This property will become useful in our construction of the measure on $S$, where it is used to contract sufficiently nice sets $F\subseteq S$ into the unit ball. Here, $S$ is the unital level set of a positive homogeneous function to be introduced later in this section.
% Contracting group
\begin{framed}
...
\noindent The next two results follow immediately from this definition. The first result is by virtue of continuity and the Banach-Steinhaus theorem (or, the uniform boundedness principle), while the second by the continuity of the determinant with respect to the operator norm. The proof of Proposition \ref{prop:ContractingCharacterization} is presented in Appendix \ref{sec:OneParameterGroups}.
\begin{framed}
...
\end{framed}
\begin{proof}
The supposition that $\{T_r\}$ is a contracting group implies that $r^E\to \mathbf{0}$ in the operator-norm as $r\to 0$; here $\mathbf{0}$ is zero transformation. Because the determinant is continuous with respect to the operator norm, we have
\begin{equation*}
...
\end{equation*}
for all $x\in \mathbb{R}^d$ and $r>0$. Because such functions (which are not necessarily polynomials, as we will see) often show up in estimating convolution powers and their attractors, we will generalize and make precise this notion of homogeneity. In what is to follow, for a function $P:\mathbb{R}^d\to\mathbb{R}$, we shall call
\begin{equation*}
...
The forthcoming Proposition \ref{prop:PositiveHomogeneousCharacterization} relates the unital level set $S$ to the contracting group $\{r^E\}$ for a positive-definite $P$ and lists the (equivalent) defining characteristics of a \textbf{positive homogeneous function}. This characterization highlights an interplay between positive definite functions, contracting groups, and unital level sets and sets the stage for our definition of positive homogeneous functions.\\


Just before we state and prove this proposition, however, we introduce some notation and point out some basic topological objects connected to positive homogeneous functions.  Given a positive homogeneous function $P$, we define
\begin{equation*}
...
\end{equation*}
for all $|x|\geq M$. 
\item\label{cond:Contracting} For each $E\in\Exp(P)$, $\{r^E\}$ is contracting.
\item\label{cond:ThereExistsContracting} There exists $E\in\Exp(P)$ for which $\{r^E\}$ is contracting.
\item\label{cond:InfiniteLimit} $\lim_{x\to\infty}P(x)=\infty.$
\end{enumerate}
\end{proposition}
\end{framed}
\begin{proof}
...
\end{equation*}
Because $\overline{\mathbb{B}_M}$, the closure of $\mathbb{B}_M$, is compact, $\{r_k^Ey\}$ has a convergent subsequence which we also denote by $\{r_k^Ey\}$ by a slight abuse of notation. In view of the continuity of $P$ at $\eta\coloneqq\lim_{k\to\infty}r_k^Ey$, we have
\begin{equation*}
...
In the language of L. H\"{o}rmander \cite{hormander_analysis_1983}, consider a semi-elliptic polynomial of the form
...
Let $Q$ be a positive homogeneous function with exponent set $\Exp(Q)$ and unital level set $S_Q=\{\eta:Q(\eta)=1\}$. Given any $f\in C^0(S_Q)$ which is positive on $S_Q$ and $E\in \Exp(Q)$, define $P=P_{f,E,Q}:\mathbb{R}^d\to\mathbb{R}$ by
\begin{equation*}
P(x)=\begin{cases}
...
\end{equation*}
for $t\in\mathbb{R}$. $w$ is $2\pi$-periodic version of the Weierstrass function. The resulting positive homogeneous function $P$ is continuous but is nowhere differentiable. Figure \ref{fig:Weierstrass} illustrates this function $P$ alongside $Q$, and together with their associated unital level sets. We note that $S_P\neq S_Q$ and this is generally the case unless $f\equiv 1$.
...
\noindent In most situations when estimating convolution powers, we are working with a continuous, positive-definite $P$ and some $E\in \Exp(P)$. To see if $P$ is a positive homogeneous function, we can check if the spectrum of $E$ is real. The following proposition captures this result. 

\begin{framed}
\begin{proposition}\label{prop:PosHomSufficientCondition}
If $P$ is continuous, positive definite and $\Exp(P)$ contains an $E\in\End(\mathbb{R}^d)$ with real spectrum, then $\{r^E\}$ is contracting and hence all of the conditions in Proposition \ref{prop:PositiveHomogeneousCharacterization} are (simultaneously) met. 
\end{proposition}
\end{framed}
\begin{proof}
...
P(Ox)=P(x)
\end{equation*}
for all $x\in\mathbb{R}^d$. By virtue of the positive-definiteness of $P$, it is easy to see that $\Sym(P)$ is a subgroup of $\Gl(\mathbb{R}^d)$. For this reason, $\Sym(P)$ is said to be the \textbf{symmetry group associated to $P$}. It turns out that $\Sym{(P)}$ is a subgroup of $\text{SL}^{\pm}(\mathbb{R}^d)$, the group of matrices with determinant $\pm 1$. Later, we will show later that the measures of sufficiently nice sets on $S$ are invariant under transformation by any element of $\Sym(P)$, as expected.  
...
For each positive homogeneous function $P$, $\Sym(P)$ is a compact subgroup of $\Gl(\mathbb{R}^d)$. Further, it is a subgroup of $\text{SL}^{\pm}(\mathbb{R}^d)$.
\end{proposition}
\end{framed}
...
Now, we want to show that every element of $\Sym{(P)}$ has determinant $\pm 1$. To this end, assume to reach a contradiction that this is not the case. By the homomorphism property of the determinant, there must exist some $O\in \Sym(P)$ for which $\abs{\det(O)}>1$. Consider the sequence $\{ O_n\} \coloneqq \{ O^n \} \subseteq \Sym(P)$. Since $\Sym(P)$ is compact, there exists a convergent subsequence $\{ O_{n_k} \} = \{ O^{n_k} \} \to O'\in \Sym(P)$. It follows that
\begin{equation*}
    \abs{\det (O')} = \lim_{n_k \to \infty}\abs{\det(O)}^{n_k} = \infty,
\end{equation*}
which is impossible because this contradicts the continuity of the determinant.
\end{proof}

\begin{remark}
A stronger statement than the second part of Proposition \ref{prop:SymCompact} would be ``$\Sym(P)$ is conjugated to a subgroup of $O(\mathbb{R}^d)$.'' However, Proposition \ref{prop:SymCompact} as it appears is sufficient for our purposes.
\end{remark}
...
\begin{equation*}
1=\abs{\det(O_r)}=\abs{\det(r^{E}r^{-E'})} = \abs{\det(r^{E})\det(r^{-E'})}=\abs{r^{\tr E-\tr E'}}=r^{\tr E-\tr E'}
\end{equation*}
for all $r>0$ and therefore $\tr E=\tr E'$.
\end{proof}

\noindent In view of the preceding corollary, to each positive homogeneous function $P$, we define the \textbf{homogeneous order of $P$} to be the unique positive number $\mu_P$ for which
\begin{equation*}
\mu_P=\tr E
\end{equation*}
for all $E\in\Exp(P)$. \\
...
\begin{equation*}
    OF=\{O\eta:\eta\in F\}.
\end{equation*}
The following proposition will be useful when we show that the measure on $S$ (which we will construct from some $E\in \Exp(P)$) is in fact independent of the choice of $E\in \Exp(P)$. In other words, we will show that the measure is intrinsic to $P$. 
...
\begin{proof}
Since $\Sym(P)$ is a subgroup of $\text{SL}^{\pm}(\mathbb{R}^d)$, for $O\in\Sym(P)$ and $E\in \Exp(P)$, we have $O^{-1}\in\Sym{(P)}$ and therefore
\begin{equation*}
    P(r^{O^{-1}E O} x) = P(O^{-1} r^E O x) = P(r^E O x) = rP(Ox) = rP(x),
\end{equation*}
for all  $r > 0$ and $x\in \mathbb{R}^d$. Thus, $O^{-1}\Exp(P) O \subseteq \Exp(P)$. Moreover, any $E\in \Exp(P)$ can be written as $O^{-1} (O E O^{-1}) O$, where $OEO^{-1}\in \Exp(P)$ because
\begin{equation*}
    P(r^{OE O^{-1}} x) = P(O r^E O^{-1} x) = P(r^E O^{-1} x) = rP(O^{-1}x) = rP(x).
\end{equation*}
Thus, $\Exp(P)\subseteq O^{-1}\Exp(P) O$, and hence $\Exp(P) = O^{-1} \Exp(P) O$ as desired. 
\end{proof}
...
\end{enumerate}
When the endomorphism $E$ is fixed, we will say that $Q$ is subhomogeneous if it is subhomogeneous with respect to $E$. Also, we will say that $Q$ is $k$-strongly subhomogeneous if it is strongly subhomogeneous with respect to $E$ of order $k$.
\end{definition}
\end{framed}
...
\end{equation*}
for all $L\in\mathcal{L}$ and $F\in\Sigma_{P,E}$. We shall denote by $((0,\infty)\times S,(\mathcal{L}\times\Sigma_{P,E})', \lambda_P\times\sigma_{P,E})$ the completion of the measure space $((0,\infty)\times S,\mathcal{L}\times\Sigma_{P,E},\lambda_P \times\sigma_P)$. With this, we state (without proving) the following Fubini-Tonelli theorem associated to $\lambda_P\times \sigma_{P,E}$, which combines the results of the Fubini-Tonelli Theorem and the ``measureable slice'' theorem (see \cite{folland_real_2013}). This allows us to exchange the order of integration, which is crucial for future applications.
\begin{framed}
...
\begin{framed}
\begin{lemma}\label{lem:SLInvariance}
Given any $O\in  \text{SL}^{\pm}(\mathbb{R}^d)$ and Lebesgue measurable $F\subseteq\mathbb{R}^d$,
\begin{equation*}
    m(OF)=m(F).
\end{equation*}
\end{lemma}
...
\begin{proof}
This lemma can be proved using the argument provided in the proof of Lemma \ref{lemma:Scaling}. 
\end{proof}
...
\begin{proof}
Let $O\in\Sym(P)$, $F\in\Sigma_P$ and, for $E\in \Exp(P)$, define $E'=O^{-1} EO$. In view of Proposition \ref{prop:ExpP}, we note that $E'\in \Exp(P)$. Observe that
\begin{equation}\label{eq:TildeConjugation}
    \widetilde{(OF)_E}=\bigcup_{0<r<1}r^E (OF)=\bigcup_{0<r<1}O\left(O^{-1} r^E O F\right)=O\left(\bigcup_{0<r<1} r^{E'}F\right)=O \widetilde{F_{E'}}
\end{equation}
thanks to Proposition \ref{prop:ContinuousGroupProperties}.
In view of Proposition \ref{prop:Endependence}, we have $O\widetilde{F_{E'}}\in \mathcal{M}_d$ because $F\in \Sigma_P=\Sigma_{P,E'}$ and $O$ is linear. Using \eqref{eq:TildeConjugation}, we find that $\widetilde{(OF)_E}\in\mathcal{M}_d$ and therefore  $OF\in\Sigma_{P,E}=\Sigma_P$ by virtue of Proposition \ref{prop:Endependence}. Moreover, we have
\begin{equation*}
...
\end{equation*}
where the third equality follows from Lemma \ref{lem:SLInvariance}. A final appeal to Proposition \ref{prop:Endependence} guarantees that
\begin{equation*}
...
\end{equation*}
Since $\Phi'$ is monotonic, the absolute value in the second term of the right hand side can be brought outside of the integral. Subsequently, we apply a fundamental theorem of calculus to this term to obtain the first assertion by taking
\begin{equation*}
...
This section can be summarized as follows. We first give some historical context related to the study of convolution powers and define the class of aforementioned ``sufficiently nice'' functions, denoted by $\mathcal{S}_d$, whose convolution power estimates we wish to obtain. This class of functions, a subspace of  $\ell^1(\mathbb{Z}^d)$, is a discrete analogue of the Schwartz class. Since our approach relies heavily on the relationship between the Fourier transform and the convolution product, we define both operations for all functions in $\ell^1(\mathbb{Z}^d)$.  From here, with the assumption that $\phi\in\mathcal{S}_d$, we can  consider the Taylor expansions $\Gamma_{\xi_0}$ of the Fourier transform of $\phi$, denoted by $\widehat{\phi}$, around the points $\xi_0$ where $\widehat{\phi}$ is maximized. As we will see, the leading polynomials in $\Gamma_{\xi_0}$ are related to the positive homogeneous functions in Chapter \ref{chap:pos-hom-fns} and its subtypes (subhomogeneous and strongly subhomogeneous of order $k$) and give rise to a homogeneous order $\mu_{\xi_0}$ depending on the ``type'' of $\xi_0$. We then show that the smallest homogeneous order over all admissible $\xi_0$'s dictates the power-law decay of the sup-norm of the convolution powers $\phi^{(n)}$.\\
...
\vspace{.3cm}
\begin{tabular}{lll}
$\bullet$ $1^E=\mathbb{I}$ &  $\bullet$ $t^{E^*}={(t^E)}^*$ & $\bullet$ $(t^E)^{-1}=t^{-E}$ 
\\
\vspace{.1cm}\\
...
\begin{equation*}
\|t^D\| = \max(\{t^{\lambda_1},t^{\lambda_2},\dots,t^{\lambda_d}\})
=\begin{cases}
...
Let $\{T_t\}_{t>0}\subseteq\Gl(\mathbb{R}^d)$ be a continuous one-parameter contracting group. There holds the following:
\begin{enumerate}[label=(\alph*), ref=(\alph*)]
...
\end{equation*}
\item\label{item:ScaleFromSphere_2} For each sequence $\{x_n\}\subseteq\mathbb{R}^d\setminus\{0\}$ such that $\lim_n|x_n|=\infty$, $x_n=T_{t_n}\eta_n$ for each $n$, where $\{\eta_n\}\subseteq \mathbb{S}$ and $t_n\rightarrow\infty$ as $n\rightarrow\infty$.
\item\label{item:ScaleFromSphere_3} For each sequence $\{x_n\}\subseteq\mathbb{R}^d\setminus\{0\}$ such that $\lim_n|x_n|=0$, $x_n=T_{t_n}\eta_n$ for each $n$, where $\{\eta_n\}\subseteq \mathbb{S}$ and $t_n\rightarrow 0$ as $n\rightarrow\infty$.
\end{enumerate}
\end{proposition}
\begin{proof}
In view of Proposition \ref{prop:ContractingLimits}, the assertion \ref{item:ScaleFromSphere_1} is a straightforward application of the intermediate value theorem to $|T_{1/t}(x)|$. For \ref{item:ScaleFromSphere_2}, suppose that $\{x_n\}\subseteq\mathbb{R}^d$ is such that $|x_n|\rightarrow \infty$ as $n\rightarrow\infty$. In view of \ref{item:ScaleFromSphere_1}, take $\{\eta_n\}\subseteq S$ and $\{t_n\}\subseteq (0,\infty)$ for which $x_n=T_{t_n}\eta_n$ for each $n$. In view of Lemma \ref{lem:OperatorBoundsforContractingGroup},
\begin{equation*}
...
\begin{equation*}
\lim_{k\rightarrow\infty}|T_{s_k}\zeta_k|\leq\lim_{k\rightarrow \infty}|T_{s_k}(\zeta_k-\zeta)|+\lim_{k\rightarrow\infty}|T_{s_k}\zeta|\leq C
\lim_{k\rightarrow\infty}|\zeta_k-\zeta|+0=0
\end{equation*}
...
% \textcolor{red}{Can you change this to include the definition of algebra of sets too? It's used in the Monotone Class lemma}
...
% Let $X$ be a topological space \st{(or a metric space, or a subset of $\mathbb{R}^n$)}\textcolor{red}{e.g., a metric space}, so that we have ``open sets.'' The Borel $\sigma$-algebra $\mathcal{B}(X)$ is the smallest $\sigma$-algebra generated by the open sets.
% \end{definition}
...
% % \end{definition}
% \textcolor{red}{We don't have the definition of algebra.}
