...
For any $\alpha>0$, the $\alpha$th-power of the Euclidean norm $x\mapsto |x|^\alpha$ is positive homogeneous.  In this case, the unital level set $S$ is the standard unit sphere $\mathbb{S}=\mathbb{S}_1(0)$, $B=\mathbb{B}=\mathbb{B}_1(0)$, and
\begin{equation*}
...
\end{equation*}
defined for $(x,y)\in\mathbb{R}^2$. It is straightforward to see that $P_1$ and $P_2$ are both semi-elliptic of the form \eqref{eq:SemiEllipticIntro} with $\mathbf{n}=(2,4)$ and positive-definite. Figure \ref{fig:PoneAndtwo} illustrates $P_1$ and $P_2$ along with their associated unital level sets $S_1$ and $S_2$ and corresponding sets $B_1=\{\xi\in\mathbb{R}^d:P_1(\xi)<1$ and $B_2=\{\xi\in\mathbb{R}^d:P_2(\xi)<1\}$ written with a slight abuse of notation.
\begin{figure}[!htb]
    \centering
    \hspace{10pt}
...
\begin{figure}[!htb]
    ...
    \vspace{-10pt}
    \includegraphics[scale=0.5]{fig_SQ.eps}
    %\caption{}
...
\item\label{property:Invariance} For any $F\in\Sigma$ and $O\in\Sym(P)$, $\sigma_P(OF)=\sigma_P(F)$.
...
\begin{equation*}
\int_S f(\eta)\sigma_P(\eta)=\frac{d}{dr}\left(\int_{B_r}f(x)\,dx\right)\Big\vert_{r=1}.
\end{equation*}
\end{corollary}
\begin{proof}
In view of Theorem \ref{thm:BestIntegrationFormula}, we have
\begin{equation*}
...
\widehat{\sigma_P}(x)=\frac{d}{dr}\widehat{\chi_{B_r}}(x)\Big\vert_{r=1}\hspace{0.5cm}\mbox{and}\hspace{0.5cm}\widehat{\chi_B}(x)=\int_0^1\widehat{\sigma_P}(r^{E^*}x)r^{\mu_P-1}\,dt
\end{equation}
...
\end{cases}
\end{equation*}
for $x\in\mathbb{R}^d$ where $\chi_{(0,1)}(\cdot)$ is the indicator function of the interval $(0,1)$. If $g\in L^1(S,\Sigma_P,\sigma_p)$, then $f\in L^1(\mathbb{R}^d)$, and 
...
and setting
\begin{equation*}
\sigma_{P,E}(F)=\sigma_P\cdot m(\widetilde{F})
\end{equation*}
...
\end{eqnarray*}
where $\al_{\xi_0} \in \mathbb{R}^d$;   $Q_{\xi_0} and $ $R_{\xi_0}$ are real-valued polynomials for which $\abs{Q_{\xi_0}}$ and $R_{\xi_0}$ are positive-definite; and  $\widetilde{Q}_{\xi_0},$ and $\widetilde{R}_{\xi_0}$ are real multivariate power series which are absolutely and uniformly convergent on $\mathcal{U}$. Then $\xi_0$ is of imaginary-homogeneous type for $\widehat{\phi}$ with associated positive-homogeneous function $|Q_{\xi_0}|$ and homogeneous order
\begin{equation*}
...
\end{equation*}
for all multi-indices $\beta$ for which $|\beta:2\mathbf{m}> 4$ and $0\leq r\leq \delta_2$. To see this, it is sufficient to take $\delta_2 = 2^{-4}$. Then because $|\beta:8\mathbf{m}| = |\beta:2\mathbf{m}|/4 > 1$,  we have that
\begin{equation*}
    |\beta:2\mathbf{m}|r^{|\beta:8\mathbf{m}|}<   |\beta:2\mathbf{m}|2^{-4|\beta:8\mathbf{m}|} = |\beta:2\mathbf{m}|2^{-|\beta:2\mathbf{m}|} < 1
\end{equation*}
for all $\abs{\beta: 2\mathbf{m}} \geq 4$. 

With these, we let $0<\delta\leq\min\{\delta_1,\delta_2\}$ be chosen so that for all $0 < r < \delta$,
\begin{equation*}
...
%% EXAMPLE 5
\begin{example}\normalfont
\textcolor{blue}{Perhaps comment on non-Gaussian behavior in the discussion of this example?} Consider the function $\phi : \mathbb{Z}^2 \to \mathbb{C}$ defined by 
\begin{equation*}
    \phi(x,y) =
    \frac{1}{192}
    \begin{cases}
...
-i\lp - \frac{x^4}{144}  - \frac{y^4}{144} - \frac{xy^3}{288} + \frac{31 x^2y^4}{27648} - \frac{x^3y}{288} + \frac{95x^3y^3}{331776} + \dots \rp \\
&& 
-\lp \frac{5y^4}{288} - \frac{11x^2y^2}{1536} + \frac{5x^4}{288}  \rp 
- \lp -\frac{xy^3}{576} + \frac{x^2y^4}{1536} - \f{x^3y}{576} + \frac{x^3y^3}{1728} + \dots \rp.
\end{eqnarray*}
In view of Proposition \ref{prop:ExpandGamma}, we can make the following identifications:
\begin{eqnarray*}
Q_{\xi_0 = 0}(x,y)
&=&   \frac{x^2}{12} +\frac{xy}{48} + \frac{y^2}{12}\\
\widetilde{Q}_{\xi_0 = 0}(x,y)
&=&  - \frac{x^4}{144}  - \frac{y^4}{144} - \frac{xy^3}{288} + \frac{31 x^2y^4}{27648} - \frac{x^3y}{288} + \frac{95x^3y^3}{331776} + \dots \\
R_{\xi_0 = 0}(x,y)
&=&   \frac{5y^4}{288} - \frac{11x^2y^2}{1536} + \frac{5x^4}{288}  \\
\widetilde{R}_{\xi_0 = 0}(x,y)
&=&  -\frac{xy^3}{576} + \frac{x^2y^4}{1536} - \f{x^3y}{576} + \frac{x^3y^3}{1728} + \dots 
\end{eqnarray*}
Consider $\mathbf{m} = (1,1)$ and $k=2$. Following Proposition \ref{prop:ExpandGamma} we have $E = \diag(1/2, 1/2)$, from which we readily find that $Q_{\xi_0=0}$ is homogeneous with respect to $E$ and $R_{\xi_0 = 0}$ with respect to $E/k$, and that $\widetilde{Q}_{\xi_0 = 0}$ and $\widetilde{R}_{\xi_0} = 0$, which are absolutely and uniformly convergent since $\Gamma_{\xi_0}(\xi)$ is $\mathbb{C}^d$-analytic, are strongly subhomogeneous with respect to $E$ and $E/k$, respectively. \\

\noindent Next, we compute the convolution powers, $\phi^{(n)}$. Figures \ref{fig:Conv_Pwr_0a} and \ref{fig:Conv_Pwr_0b} show the real part of $\phi^{(n)}$ for $n = 100, 1000$. 
To study the decay of $\abs{\phi^{(n)}}$, we consider the compact set $K = [-300, 300] \times [-300, 300] \subseteq \mathbb{R}^2$.  Figure \ref{fig:Conv_Pwr_0e} shows the power-law decay of $\max_{x\in K}\abs{\phi^{(n)}(x)}$ for $1 \leq n\leq 700$. In figure  \ref{fig:Conv_Pwr_0f}, we plot $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ against $n$. We observe that as $n$ grows large, the quantity $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ approaches a constant in accordance with Theorem \ref{thm:ConvolutionPowerEstimate}.

\begin{figure}[!htb]
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{conv_ex0_100.eps}
    \caption{$\Re{\phi^{(n)}}$ for $n = 100$.}
    \label{fig:Conv_Pwr_0a}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{conv_ex0_1000.eps}
    \caption{$\Re{\phi^{(n)}}$ for $n = 1000$.}
    \label{fig:Conv_Pwr_0b}
    \end{subfigure}
\end{figure}
...
\begin{figure}[!htb]
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{max01.eps}
    \caption{$\max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 700$}
    \label{fig:Conv_Pwr_0e}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{max02.eps}
    \caption{$n^{\mu_P} \max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 700$}
    \label{fig:Conv_Pwr_0f}
    \end{subfigure}
\end{figure}
...
\noindent Next, we compute the convolution powers, $\phi^{(n)}$. Figures \ref{fig:Conv_Pwr_2a} and \ref{fig:Conv_Pwr_2b} show the real part of $\phi^{(n)}$ for $n = 100, 400$. To study the decay of $\abs{\phi^{(n)}}$, we consider the compact set $K = [-50, 50] \times [-50, 50] \subseteq \mathbb{R}^2$.  Figure \ref{fig:Conv_Pwr_2e} shows the power-law decay of $\max_{x\in K}\abs{\phi^{(n)}(x)}$ for $1 \leq n\leq 1000$. In figure  \ref{fig:Conv_Pwr_2f}, we plot $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ against $n$. We observe that as $n$ grows large, the quantity $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ approaches a constant in accordance with Theorem \ref{thm:ConvolutionPowerEstimate}.
...
\begin{figure}[!htb]
    ...
    \caption{$\Re{\phi^{(n)}}$ for $n = 100$.}
    \label{fig:Conv_Pwr_2a}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{conv_ex_b.eps}
    \caption{$\Re{\phi^{(n)}}$ for $n = 400$.}
...
\begin{figure}[!htb]
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{maxK1.eps}
    \caption{$\max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 1000$}
    \label{fig:Conv_Pwr_2e}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{maxK2.eps}
    \caption{$n^{\mu_P} \max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 1000$}
    \label{fig:Conv_Pwr_2f}
    \end{subfigure}
\end{figure}
...
\noindent Next, we compute the convolution powers, $\phi^{(n)}$. Figures \ref{fig:Conv_Pwr_1a} and \ref{fig:Conv_Pwr_1b} show the real part of $\phi^{(n)}$ for $n = 300, 600$. To study the decay of $\abs{\phi^{(n)}}$, we consider the compact set $K = [-50, 50] \times [-50, 50] \subseteq \mathbb{R}^2$.  Figure \ref{fig:Conv_Pwr_1e} shows the power-law decay of $\max_{x\in K}\abs{\phi^{(n)}(x)}$ for $1 \leq n\leq 1000$. In figure  \ref{fig:Conv_Pwr_1f}, we plot $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ against $n$. We observe that as $n$ grows large, the quantity $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ approaches a constant in accordance with Theorem \ref{thm:ConvolutionPowerEstimate}.
...
\begin{figure}[!htb]
    ...
    \caption{$\Re{\phi^{(n)}}$ for $n = 300$.}
    \label{fig:Conv_Pwr_1a}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{conv_ex1.eps}
    \caption{$\Re{\phi^{(n)}}$ for $n = 600$.}
    \label{fig:Conv_Pwr_1b}
    \end{subfigure}
\end{figure}
...
\begin{figure}[!htb]
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{max21.eps}
    \caption{$\max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 1000$}
    \label{fig:Conv_Pwr_1e}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{max22.eps}
    \caption{$n^{\mu_P} \max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 1000$}
    \label{fig:Conv_Pwr_1f}
    \end{subfigure}
\end{figure}
\end{example}
...
&=& 
-i
\lp \frac{x^2}{48} - \frac{xy^4}{192} +\frac{ y^8}{384}  \rp
-i
\lp -\frac{xy^8}{960} - \frac{x^4}{576} +\frac{xy^6}{288} + \frac{x^3y^4}{1152} - \frac{x^3y^6}{1728} + \frac{x^3y^8}{5760} + \frac{31 x^4y^8}{589824} + \dots\rp \\
&& 
- \lp \frac{95x^4}{4608} - \frac{5x^2 y^8}{73728} + \frac{y^{16}}{589824} \rp 
- \lp \frac{x^3y^4}{9216} - \frac{x^3y^6}{13824} +\frac{x^3y^8}{46080} + \frac{x^4y^8}{110592} + \dots \rp
\end{eqnarray*}
In view of Proposition \ref{prop:ExpandGamma}, we can make the following identifications:
\begin{eqnarray*}
Q_{\xi_0 = 0}(x,y)
...
R_{\xi_0 = 0}(x,y)
&=&   \frac{95x^4}{4608} - \frac{5x^2 y^8}{73728} + \frac{y^{16}}{589824} \\
\widetilde{R}_{\xi_0 = 0}(x,y)
&=&  \frac{x^3y^4}{9216} - \frac{x^3y^6}{13824} +\frac{x^3y^8}{46080} + \frac{x^4y^8}{110592} + \dots.
\end{eqnarray*}
Consider $\mathbf{m} = (1,4)$ and $k=2$. Following Proposition \ref{prop:ExpandGamma} we have $E = \diag(1/2, 1/8)$, from which we readily find that $Q_{\xi_0=0}$ is homogeneous with respect to $E$ and $R_{\xi_0 = 0}$ with respect to $E/k$, and that $\widetilde{Q}_{\xi_0 = 0}$ and $\widetilde{R}_{\xi_0} = 0$ are strongly subhomogeneous with respect to $E$ and $E/k$, respectively.\\

\noindent Next, we compute the convolution powers, $\phi^{(n)}$. Figures \ref{fig:Conv_Pwr_3a} and \ref{fig:Conv_Pwr_3b} show the real part of $\phi^{(n)}$ for $n = 100, 400$. To study the decay of $\abs{\phi^{(n)}}$, we consider the compact set $K = [-750, 750] \times [-750, 750] \subseteq \mathbb{R}^2$.  Figure \ref{fig:Conv_Pwr_3e} shows the power-law decay of $\max_{x\in K}\abs{\phi^{(n)}(x)}$ for $1 \leq n\leq 1500$. In figure  \ref{fig:Conv_Pwr_3f}, we plot $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ against $n$. We observe that as $n$ grows large, the quantity $n^{\mu_P}\max_{x\in K}\abs{\phi^{(n)}(x)}$ approaches a constant in accordance with Theorem \ref{thm:ConvolutionPowerEstimate}.
...
\begin{figure}[!htb]
    ...
    \caption{$\Re{\phi^{(n)}}$ for $n = 100$.}
    \label{fig:Conv_Pwr_3a}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{conv400.eps}
    \caption{$\Re{\phi^{(n)}}$ for $n = 400$.}
    \label{fig:Conv_Pwr_3b}
    \end{subfigure}
\end{figure}
...
\begin{figure}[!htb]
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{max41.eps}
    \caption{$\max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 1500$}
    \label{fig:Conv_Pwr_3e}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[scale=0.58]{max42.eps}
    \caption{$n^{\mu_P} \max_{x\in K}\abs{\phi^{(n)}(x)}$ versus $1 \leq n\leq 1500 $}
    \label{fig:Conv_Pwr_3f}
    \end{subfigure}
\end{figure}
\end{example}
...
\begin{theorem}[Theorem 8.12 of \cite{rudin_real_1987}]\label{thm:Fubini}
...
We have:
\begin{enumerate}
    \item\label{item:Complete} $(S,\Sigma_E,\sigma_{P,E})$ is the completion of the measure space $(S,\mathcal{B}(S),\sigma_{P,E})$. In particular, the measure space $(S,\Sigma_E,\sigma_{P,E})$ is complete and every $F\in \Sigma_E$ is of the form $F=G\cup H$ where $G$ is a Borel set and $H$ is a subset of a Borel set $Z$ with $\sigma_{P,E}(Z)=0$.
...
\section{Continuous one-pameter subgroups of $\Gl(\mathbb{R}^d)$}\label{sec:Appendix}
