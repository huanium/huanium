...
% $P=P_{\xi_0}$ is positive-homogeneous and, for $E\in \Exp(P)$, we have
% \begin{equation*}
%     \widetilde{P}(r^E\xi)=o(r)
% \end{equation*}
% and
% \begin{equation*}
%     \partial_r\widetilde{P}(r^E\xi)=o(1)
% \end{equation*}
% for each $\xi$.
% % and how is \tilde{P} defined?

% This would really be the defining thing. I believe the first property, alone, characterizes that its of "higher" order than P.

% wait, so let me get this right.
% We first have $\Gamma(\xi)$, right?
% Then we pick out $P$ from $\Gamma$?
% This $P$ should be pos-hom.

% Yes. Should I try to work with the definition we were using? THis probably would make things more clear.
% I also want to point out that a similar circle of ideas appears in Proposition 3.3 in my $Z^d$ paper. The whole thing is a pain in the ass, but the ideas are correct. Page 21 in that paper is the relevant page.

% Ah i see

% The question is essentially this: Let $\Gamma$ be given by the expansion. In general, it's just a Taylor expansion. Then, how does one know when there is a dominant (on the low order) polynomial which is pos-hom and then its difficult to say exactly what is a higher order. I guess, defining order in a concrete way is a big issue.
...
% the definition we had and give something try. A goal for you is to think hard about whether or not our examples fits it. Sound good?
...
%% can we go to the example.tex file?
%% i want compile this without messing up V7
...
% \end{equation*}
% for $n\in\mathbb{N}_+$, and \textcolor{blue}{blah!}. Then
% \begin{equation*}
%     \|g_{n,\eta}\|_{L^{\infty}[1/n^{\mu},\delta^\mu]}\leq 1
% \end{equation*}
% and
% \begin{equation*}
% \|\partial_\theta g_{n,\eta}\|_{L^1[1/n^{\mu},\delta^\mu]}\leq 1
% \end{equation*}
% for all $\eta\in S$ and $n\in\mathbb{N}_+$.  
% \end{lemma}
...
\end{lemma}
\begin{proof}
In studying the proof of Lemma \ref{lem:EstPosHom}, we see that without loss of generality we may assume that $\xi_0=0$ and $\widehat{\phi}(0)=1$. Let $G\in \Exp(Q)$ (and so $E=kG\in \Exp(P)$) % and let $\delta>0$ be as guaranteed by Lemma \ref{lem:LwrBndDerivF}
...
\newblock The Journal of Fourier Analysis and Applications 21(4) 754-798 (2015).}
