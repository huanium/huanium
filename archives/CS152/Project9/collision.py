...
# Updates the ball's position and returns true if there was a collision.
# Returns False if there was no collision (balla still needs to be udpated).
...
def collision_ball_letter(ball, letter, dt):
    #print('something')
    for block in letter.getVis():
        collision_ball_block(ball, block, dt)
...
collision_router[('ball', 'letter')] = collision_ball_letter


def collision(thing1, thing2, dt):
    tup = (thing1.getType(), thing2.getType())
    result = collision_router[tup]
    result(thing1, thing2, dt)
