...
selected = {};
selected(1:2,1:3) = data(1:2,1:3); j=3;
for i = 3:size(data,1)
    if data{i,4}, selected(j,1:3) = data(i,1:3); j=j+1; end
end
handles.props = data;
selected = selected';
outp.props = selected;
outp.pnames = selected(1,:);
outp.widths = selected(2,:);
outp.editables = selected(3,:);
outp.update = 1;
handles.output = outp;
guidata(hObject, handles);
...
% handles    structure with handles and user data (see GUIDATA)
data = get(handles.Property_Table,'Data');
handles.props = [data;{'Prop Name',50,false,true}];
set(handles.Property_Table,'Data',handles.props);
guidata(hObject, handles);
...
% Settings file doesnt exist
    props = {...
        'num','name', 'hide', 'notes','RF23';... % column name
