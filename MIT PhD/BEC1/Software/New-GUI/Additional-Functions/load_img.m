function [ absimg, allimgs ] = load_img( file_path, replace_bad_pixels, subsample )
...
for i = 1:length(allimgs)
    allimgs{i} = imresize(allimgs{i}, 1/subsample, 'method', 'box');
%     allimgs{i} = imresize(allimgs{i}, 'method', 'box');
end
allimgs{1} = (allimgs{4}-allimgs{5}) ./ (allimgs{3}-allimgs{5});
allimgs{2} = allimgs{1};
absimg = allimgs{1};
