function DipoleOscillationMeanFieldPrediction(Bose_XZ, BEC_X,BEC_Y,BEC_Z,varargin)
...
%% calculate the mean-field frequency shift
xTF_ = xTF*1e-6;
yTF_ = yTF*1e-6;
zTF_ = zTF*1e-6;

a_inverse = linspace(-1,1);
mreduce = mNa*mK/(mNa+mK);

g_coupling = @(a) 2*pi*hbar^2*a*aBohr/mreduce; %input a in unit of aBohr
nc0 = n_Na(0,0,0)*(1e6)^3;
chemical_potential_fun = @(a) g_coupling(a)*nc0;
omegaMF = @(a_inverse) sqrt(160^2-chemical_potential_fun(1./a_inverse)*2/mK/yTF_^2);

% omegaMF = @(a) sqrt(160^2-15*hbar^2*a/(2*xTF_*zTF_*yTF_^3*mK*mreduce));

figure(1);clf;
plot(a_inverse,omegaMF(a_inverse));
xlabel('1/a (a_{Bohr}^{-1})');
ylabel('frequency Hz');