function [param_fit, tempCi] = chemicalPotentialBECTOFfreeOmega(params, widths,widthCis,excludeIdx)
...
pGuess  = [2,100];
ub      = [100,1000];
fitfun = @(p,x) 10^6*sqrt(p(1).*10^3./mass.*PlanckConst.*2.*(1+(2*pi*p(2)).^2.*x.^2))./(2*pi*p(2));
...
title(['\mu(kHz) = ', num2str(param_fit(1)),' [', num2str(tempCi(1,1)),' - ', num2str(tempCi(1,2)),'] F(Hz) = ', num2str(trapfreq),' [', num2str(tempCi(2,1)),' - ', num2str(tempCi(2,2)),']' ]);
% Label axes
xlabel time(s)
ylabel width(\mum)
grid on
sqrt((param_fit(1))*PlanckConst*1000/mass)*1000
