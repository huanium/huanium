...
% Last Modified: 15.06.2016
    ...
    MonteCarlo_length = floor(size(params,2)/10)*10; % need to be a multiple of 10
    funVals = zeros(1,MonteCarlo_length);
    if p.Results.plot
        fprintf('Evaluating function values: %3.0f%%\n',0);
    end
    for idx = 1:10
        for kdx = 1:ceil(MonteCarlo_length/10)
            funValIdx = (idx-1)*ceil(MonteCarlo_length/10)+kdx;
    ...
    mask = isnan(funVals);
    allSame = all(~diff(real(funVals(~mask))));
    
    if(allSame)
        funValue = funVals(1);
        funCI = [funVals(1), funVals(1)];
        mcSamples = funVals;
    else
    ...
    %             fprintf('extend left ');
        ...
        if p.Results.plot
            figure(776);
            idxP = find(vals_bins*MonteCarlo_length>2, 1, 'first') : find(vals_bins*MonteCarlo_length>2, 1, 'last');
