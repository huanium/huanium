function [analysis]=generateLineDensityLinearResponse(params,Na_analysis,varargin)
...
%     NaHighTFThresholdX = 30;
    NaLowTFThresholdX = 32;
    NaHighTFThresholdX = 45;
    ...
    % exclude out the BEC size that not within range
    BEC_size_logic = not(Na_analysis.analysis.fitBimodalExcludeCenter.xparam(:,2) > NaLowTFThresholdX...
                            & Na_analysis.analysis.fitBimodalExcludeCenter.xparam(:,2) < NaHighTFThresholdX);
    
%     Na_analysis.badShots = [Na_analysis.badShots, filenames(BEC_size_logic)];
...
%             imageIdx = trueAnalyzedImageIdx(find(Na_analysis.parameters==freq & BEC_size_logic'));
...
%             imageIdx = trueAnalyzedImageIdx(bootIdx(find(Na_analysis.parameters(bootIdx)==freq & BEC_size_logic')));
...
%             disp(['no images with freqency ' num2str(freq)])
        else
...
%             binaryMask = or(binaryMask1,binaryMask2);
            ...
            binaryMask = and(NaLowTFThresholdX<Na_analysis.analysis.fitBimodalExcludeCenter.xparam(RefPosIdx,2),...
                             Na_analysis.analysis.fitBimodalExcludeCenter.xparam(RefPosIdx,2)<NaHighTFThresholdX);
            
%             measM92.loadNewestSPEImage(params(idx));
            ...
            measM92.fitIntegratedGaussian('last');
