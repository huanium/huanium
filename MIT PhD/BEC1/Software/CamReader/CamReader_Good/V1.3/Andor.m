function varargout = Andor(varargin)
% ANDOR MATLAB code for Andor.fig
%      ANDOR, by itself, creates a new ANDOR or raises the existing
%      singleton*.
%
%      H = ANDOR returns the handle to a new ANDOR or the handle to
%      the existing singleton*.
%
%      ANDOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ANDOR.M with the given input arguments.
%
%      ANDOR('Property','Value',...) creates a new ANDOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Andor_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Andor_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Andor

% Last Modified by GUIDE v2.5 09-Oct-2015 00:30:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Andor_OpeningFcn, ...
                   'gui_OutputFcn',  @Andor_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Andor is made visible.
function Andor_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Andor (see VARARGIN)

% Choose default command line output for Andor
...
% UIWAIT makes Andor wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Andor_OutputFcn(hObject, eventdata, handles) 
...
while get(handles.Scanning,'value')
    NewFileList=dir([handles.infolder,'\*.aia']);
    N=size(NewFileList,1);
    newnamelist={};
    pause(0.2);
    for i=1:N
        temp=NewFileList(i);
        newnamelist=[temp.name,newnamelist];
        name=temp.name(1:end-4);
        name=[name,'fits'];
        tempimg=readaia([handles.infolder,'\',temp.name]);
        fitswrite(tempimg,[handles.outfolder,'\',name]);
    ...
    if N>0
        ...
        num=var2-1;
        img=fitsread([handles.outfolder,'\',imgname]);
        if num==0
            frame=real(-log((img(:,:,1)-img(:,:,3))./(img(:,:,2)-img(:,:,3))));
            if get(handles.AutoScaleRaw,'value')
                set(handles.Custom,'value',1);
    ...
    set(handles.Imagelist,'String',handles.list);
    guidata(hObject,handles);
    pause(0.1);
end
guidata(hObject,handles);
