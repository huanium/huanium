...
\begin{enumerate}[label=(\alph*)]
	...
	\begin{align*}
		...
		=& \,\boxed{\f{5}{8}} \quad\quad \text{(simplified in Mathematica)}
	\end{align*}

	Mathematica code:
	\begin{lstlisting}
		In[50]:= P = (1/2)*(Cos[\[Theta]]^6 + Sin[\[Theta]]^6) + 
		2*((1/4)*(1/8)*(Cos[\[Theta]] + Sin[\[Theta]])^6 + (1/4)*(1/
		8)*(Cos[\[Theta]] - Sin[\[Theta]])^6) // FullSimplify
