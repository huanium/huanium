...
% Last Modified by GUIDE v2.5 16-Jun-2022 00:55:47
...
handles.outfolder=['Z:\Images\', YearString, '\' ,MonthString,'\',DateString];
set(handles.OutFolder,'String',handles.outfolder);
handles.secoutfolder=['Z:\Images\Delete My Contents'];
...
handles.infolder='C:\data\TopImage';
...
if get(handles.Counting,'value') % do the atom counting
    ...
    atomcountimg=img(ymin:ymax,xmin:xmax,:);
    atomnumbermap=AtomNumber(atomcountimg,pixelsize,sigma, satcount);
    atomnumbermap(isinf(atomnumbermap)|isnan(atomnumbermap))=0;
    atomnumber=sum(sum(atomnumbermap));
    atomnumberG= 0 ; %FindAtomNumberGaussian(atomnumbermap);
    set(handles.AtomNumberG,'string',num2str(atomnumberG, '%10.3e' ));
    set(handles.AtomNumberP,'string',num2str(atomnumber, '%10.3e' ));
...
% --- Executes on button press in split_fits.
function split_fits_Callback(hObject, eventdata, handles)
% hObject    handle to split_fits (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)


% --- Executes on button press in scan_files.
function scan_files_Callback(hObject, eventdata, handles)
% hObject    handle to scan_files (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

scan_value = get(handles.scan_files,'Value');
...
while scan_value == 1
    ...
    %%% repeat task after some waiting
    pause(0.5);
    disp("Stop now or wait 2s")
    pause(1.5);
    disp("   ")
    
    % check for scan_value again...
    scan_value = get(handles.scan_files,'Value');
...
% Hint: get(hObject,'Value') returns toggle state of scan_files
