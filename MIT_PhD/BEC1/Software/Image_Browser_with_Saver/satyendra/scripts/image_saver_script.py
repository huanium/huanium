...
def main():
    ...
    image_specification_list = prompt_for_image_type_input(arg_dir)
    ...
    # object my_watchdog created. Now save it in the working folder
    with open(pickle_dump_location, 'wb') as file_my_watchdog:
        pickle.dump(my_watchdog, file_my_watchdog)
    file_my_watchdog.close()
...
def prompt_for_image_type_input(arg_dir):
...
def initialize_savefolder(saving_location_root_pathname, user_save_label):
    ...
    # At this point folder duplicate check is already done by matlab. So we can bypass this :)
