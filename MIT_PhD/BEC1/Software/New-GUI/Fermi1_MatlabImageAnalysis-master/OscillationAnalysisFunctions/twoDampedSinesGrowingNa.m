function OscAnalysis = twoDampedSinesGrowingNa(ana,varargin)
%allow 2nd freq component to exponentially grow in time
...
% %     end
    ...
    fitfun = @(p,x) p(1)*exp(-x/p(2)).*sin(2*pi*p(3)*x+p(4))+ ...
        p(5)*exp(-x/p(6)).*sin(2*pi*p(7)*x+p(8)).*(1-exp(-x/p(2))) + p(9);
    ...
    lb =    [0              , 0 ,    paramRemnant(3)*.5  , 0     ,0               ,tau2   ,0      ,paramPrelim(3)-.5 ,min(yvals)];
    ub =    [range(yvals), 1000, paramRemnant(3)*1.5, 2*pi,        range(yvals)/2, tau2, .5, paramPrelim(3)+.5, max(yvals)];
...
% %         pGuess(2)=1;
    ...
    %for now, treat low scattering lengths as special cases
%     if aIB==375
%         pGuess=[41.0000   22.0000    0.1250    2.7705   13.0000  181.8683    0.0738    4    0];
%         ub=[];lb=[];
%     end
    if beating
        if aIB==-260
