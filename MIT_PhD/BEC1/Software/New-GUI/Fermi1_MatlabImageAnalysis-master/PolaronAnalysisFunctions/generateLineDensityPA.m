function [analysis]=generateLineDensityPA(times,Na_analysis,varargin)
    ...
    analysis = struct;
    [~,NaCi,~] = propagateErrorWithMC(@(x) x,Na_analysis.analysis.fitBimodalExcludeCenter.xparam(:,2)','plot',true,'CIthreshold', NaInsituCI);
    NaLowTFThresholdX = NaCi(1);
    NaHighTFThresholdX = NaCi(2);
    NaLowTFThresholdX = 18;
    NaHighTFThresholdX = 30;
    [~,NaCi,~] = propagateErrorWithMC(@(x) x,Na_analysis.analysis.fitBimodalExcludeCenter.xparam(:,1)','plot',true,'CIthreshold', NaInsituCI);
    NaLowAmpThresholdX = NaCi(1);
    NaHighAmpThresholdX = NaCi(2);
    NaLowAmpThresholdX = 0.;
    NaHighAmpThresholdX = 100;
    
    
    % take care of bad shots
    analyzedImageIdx = 1:length(Na_analysis.parameters);
    imageIdx = 1:length(times);
    ...
    measKwithPA = Measurement('K1','imageStartKeyword','K1 m7','sortFilesBy','name','plotImage','original','NormType','Shell');
    measKwithPA.settings.normBoxOffset = 10;
    measKwithPA.settings.LineDensityFoldCenter = LineDensityFoldCenter;
    measKwithPA.settings.LineDensityPixelAveraging = LineDensityPixelAveraging;
    measKwithPA.settings.fudgeWidth = 5;
    measKwithPA.settings.fudgeFilterFreq = 7;
    measKwithPA.settings.superSamplingFactor1D = superSamplingFactor1D;
    measKwithPA.settings.diffToDarkThreshold = -2;
        
    measKnoPA = Measurement('K2','imageStartKeyword','K2 m9','sortFilesBy','name','plotImage','original','NormType','Shell');
    measKnoPA.settings.normBoxOffset = 10;
    measKnoPA.settings.normBox = measKwithPA.settings.normBox;
    measKnoPA.settings.LineDensityFoldCenter = LineDensityFoldCenter;
    measKnoPA.settings.LineDensityPixelAveraging = LineDensityPixelAveraging;
    measKnoPA.settings.fudgeWidth = 5;
    measKnoPA.settings.fudgeFilterFreq = 7;
    measKnoPA.settings.superSamplingFactor1D = superSamplingFactor1D;
    measKnoPA.settings.diffToDarkThreshold = -2;
    ...
    for idx = 1:length(uniqueFreq)
        ...
        if(isempty(imageIdx))
            ...
            measKwithPA.settings.marqueeBox = [round(ReferencePos(1,1)-fullBoxWidth/2-BECoffsetX),...
                                         round(ReferencePos(1,2)-boxHeight/2-BECoffsetY),...
                                         fullBoxWidth,boxHeight];
            measKwithPA.loadAndAverageSPEImages(freq,imageIdx,ReferencePos,'badShotImageNames',Na_analysis.badShots,...
                                            'binaryImageSelection',binaryMask);

            
            measKnoPA.settings.marqueeBox = [round(ReferencePos(1,1)-fullBoxWidth/2-BECoffsetX),...
                                         round(ReferencePos(1,2)-boxHeight/2-BECoffsetY),...
                                         fullBoxWidth,boxHeight];
            measKnoPA.loadAndAverageSPEImages(freq,imageIdx,ReferencePos,'badShotImageNames',Na_analysis.badShots,...
                                            'binaryImageSelection',binaryMask);
                                        
            measKnoPA.fitIntegratedGaussian('last');
                                        
            if(useUpsampling == true)
                m72LineDensitiesMatrix(idx,:)   = measKwithPA.lineDensities.YintegratedUpsampled(idx,:);
                m92LineDensitiesMatrix(idx,:)   = measKnoPA.lineDensities.YintegratedUpsampled(idx,:);
                KTransferMatrix(idx,:)          = RFTransferFun(measKwithPA.lineDensities.YintegratedUpsampled(idx,:),measKnoPA.lineDensities.YintegratedUpsampled(idx,:));
            
            else
                try
                    m72LineDensitiesMatrix(idx,:)   = measKwithPA.lineDensities.Yintegrated(idx,:);
                    m92LineDensitiesMatrix(idx,:)   = measKnoPA.lineDensities.Yintegrated(idx,:);
                    KTransferMatrix(idx,:)          = RFTransferFun(measKwithPA.lineDensities.Yintegrated(idx,:),measKnoPA.lineDensities.Yintegrated(idx,:));
                catch
    ...
    analysis.m72Images              = measKwithPA.images.ODImages;
    analysis.m92Images              = measKnoPA.images.ODImages;
    ...
    if(strcmp(spectrum,'ejection'))
        ...
        m92BasedKLinedensity = (squeeze(mean(m92LineDensitiesMatrix(mask,:).*shotToShotVector(mask)',1)))/m92Tom72ImagingConversion; %integrate over frequency
    else
        transferImageSimple = @(m72,m92) ((m72-offM92parameter*m92));
