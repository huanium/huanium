...
uplegAOMserialport = serial('COM12');
set(uplegAOMserialport,'BaudRate',115200);
uplegAOMserialport.TimeOut = 100e-3;
fopen(uplegAOMserialport);

downlegAOMserialport = serial('COM14');
set(downlegAOMserialport,'BaudRate',115200);
downlegAOMserialport.TimeOut = 100e-3;
fopen(downlegAOMserialport);
...
lastField = 78.4;%78.9;%80.3;
lastUplegAOM = 59;%62.03;%400;
lastDownlegAOM = 303;
lastPulseTime = 50;%0;
...
while true
    ...
    if ~isempty(newFieldString) 
        tic
        newNum = sscanf(newFieldString,'B%fuplegAOM%fdownlegAOM%fPulseTime%f')
        newFieldNum = newNum(1);
        newUplegAOM = abs(newNum(2));
        newDownlegAOM = abs(newNum(3));
        pulseTime = abs(newNum(4));
        ...
        %updata upleg AOM frequency
        if ~isempty(newUplegAOM) && newUplegAOM ~= lastUplegAOM 
            % AOM programming comman here
            setSG4400L_frequency(newUplegAOM, uplegAOMserialport);
            
            % RF sweep
%             setSG4400L_frequency_sweep(newAOM,lastAOM,AOMserialport);
            
            lastUplegAOM = newUplegAOM;
        end
        
        %update downleg AOM frequency
        if ~isempty(newDownlegAOM) && newDownlegAOM ~= lastDownlegAOM 
            % AOM programming comman here
            setSG4400L_frequency(newDownlegAOM, downlegAOMserialport);
            
            
            lastDownlegAOM = newDownlegAOM;
        end
        ...
        fprintf(['new field programmed: ' num2str(newFieldNum) ' G; ' 'new AOM ' num2str(newUplegAOM) 'MHz ' ...
                 'Pulse Duration: ' num2str(lastPulseTime) 'ns   ' datestr(datetime(now,'ConvertFrom','datenum')) '\n'])
    else
        fprintf(['last field: ' num2str(lastField) ' G; aBF = ' num2str(FBresonanceLandscapeRefined('Binterest',lastField)) ' aBohr  ' 'last AOM ' num2str(lastUplegAOM) 'MHz ' ...
                'Pulse Duration: ' num2str(lastPulseTime) 'ns   ' datestr(datetime(now,'ConvertFrom','datenum')) '\n' ])
    end
end
