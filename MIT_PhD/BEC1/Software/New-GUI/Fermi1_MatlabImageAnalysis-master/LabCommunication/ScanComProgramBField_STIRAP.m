...
%             setSG4400L_frequency_sweep(newAOM,lastAOM,AOMserialport);
        ...
        if ~isempty(STIRAPpause) && STIRAPpause ~= lastPauseTime
            programDoubleSTIRAP(1,0.00015,10,20,STIRAPpause,10)
           
            lastPauseTime = STIRAPpause;
        end
        ...
        fprintf(['new field programmed: ' num2str(newFieldNum) ' G; ' 'new AOM ' num2str(newAOM) 'MHz ' ...
                 'Pulse Duration: ' num2str(lastPauseTime) 'ns   ' datestr(datetime(now,'ConvertFrom','datenum')) '\n'])
    else
