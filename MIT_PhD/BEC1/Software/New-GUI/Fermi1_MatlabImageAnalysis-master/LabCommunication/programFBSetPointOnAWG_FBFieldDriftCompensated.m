function programFBSetPointOnAWG_FBFieldDriftCompensated(FieldValues,DurationsInMS)
...
SampleEndSecondLinear = SampleEndFirstPlateau+round(ntotalSamples*DurationsInMS(4)/DurationTotal);
...
FieldRamp(SampleEndFirstPlateau+1:SampleEndSecondLinear) = (((SampleEndFirstPlateau+1):SampleEndSecondLinear)-(SampleEndFirstPlateau+1))/(SampleEndSecondLinear-(SampleEndFirstPlateau+1))*(FieldValues(3)-(FieldValues(2)+ConstantSGByFieldOffset))+FieldValues(2)+ConstantSGByFieldOffset;

if DurationsInMS(4)>0.1
    StepInGauss = 0;
else
    StepInGauss = FieldValues(3)-FieldValues(2);
end
times   = timePerSample*(0:length(FieldRamp(SampleEndSecondLinear+1:end))-1);
amp1     = 0.03597;
drift1   = 1/0.3092;
...
FieldRamp(SampleEndSecondLinear+1:end) = FieldValues(3)+StepInGauss*amp1*exp(-times/drift1)+StepInGauss*amp2*exp(-times/drift2);
