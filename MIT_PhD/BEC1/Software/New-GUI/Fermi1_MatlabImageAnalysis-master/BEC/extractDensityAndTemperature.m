function analysis = extractDensityAndTemperature(GaussX,GaussY,GaussZ,BEC_X,BEC_Y,BEC_Z,numberAnalysis,KData,analysisBoxInPixel,numBoxes,varargin)
    ...
    for idx=0:numBoxes-1
        densityC(idx+1)=1e12*integral3(g_BEC, idx*analysisBoxInMuMeter/2,(idx+1)*analysisBoxInMuMeter/2,-ylim,ylim,-ylim,ylim)/integral3(f_BEC, idx*analysisBoxInMuMeter/2,(idx+1)*analysisBoxInMuMeter/2,-ylim,ylim,-ylim,ylim);
    ...
    for idx=0:numBoxes-1
        ...
        if(densityC(min(numBoxes,idx+2))>0)
            VarianceC(idx+1)=integral3(g_BEC, idx*analysisBoxInMuMeter/2,(idx+1)*analysisBoxInMuMeter/2,-ylim,ylim,-ylim,ylim)/integral3(f_BEC, idx*analysisBoxInMuMeter/2,(idx+1)*analysisBoxInMuMeter/2,-ylim,ylim,-ylim,ylim);
        else
            VarianceC(idx+1)=0;
        end
        if(densityT(idx+1)>0)
            VarianceT(idx+1)=integral3(g_Therm, idx*analysisBoxInMuMeter/2,(idx+1)*analysisBoxInMuMeter/2,-ylim,ylim,-ylim,ylim)/integral3(f_Therm, idx*analysisBoxInMuMeter/2,(idx+1)*analysisBoxInMuMeter/2,-ylim,ylim,-ylim,ylim);
        else
            VarianceT(idx+1)=0;
        end
    end
    ...
    legend('n_c/n from fitted amplitudes','1-(T/Tc)^{1.5}','Location', 'Best');
