function analysis = DensityAndTemperature2D(GaussX,GaussY,GaussZ,BEC_X,BEC_Y,BEC_Z,KData,boxWidths,boxHeigths,xTFpixZcam,varargin)
...
%%%%%%%%%%%%  Define constants %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    intLimit=GaussX.GaussianWidth*10*1e-6;%integration limits in meter for calculating thermal number
...
%             VarianceHigh_quad(trueBoxIdx)  = mean(VarHigh(boxIdx));
%             VarianceLow_quad(trueBoxIdx)   = mean(VarLow(boxIdx));
% 
% % %             VarianceT_quad(trueBoxIdx)      = mean(VarT(boxIdx));
