...
\begin{exmp}
	...
	\begin{sln*}
		...
		Now,
		\begin{align*}
		\int_0^t f_n(s) e^{(\alpha n \pi)^2s} \,ds &= \int_0^t \frac{4}{(n\pi)^3}(1-(-1))^n e^{(\alpha n \pi)^2s} \,ds\\
		&= \frac{4}{(n\pi)^5}(1-(-1)^n)\left(e^{(n\pi)^2t} - 1\right).
		\end{align*}
		All together...
		\begin{align*}
		u(t,x) &= \sum^\infty_{n=1} \left(e^{-(\alpha n\pi)^2t}\left(\frac{4}{(n\pi)^5}(1-(-1)^n)\left(e^{(n\pi)^2t} - 1\right) \right)\right)\sin(n\pi x) + e^{-(\pi \alpha)^2t}\sin(\pi x).
		\end{align*}
	\end{sln*}
\end{exmp}
