...
\begin{prob*}\textbf{2.}
	...
	\begin{sln*}\textbf{2.}
		...
		So
		\begin{align*}
		3 \leq \dim\left( \mathcal{A}^{-1}[\W]  \right).
		\end{align*}
		But since the dimension of any image of $\mathcal{A}$ cannot exceed 3 (because $\mathcal{A}$ is a $3\times 3$ matrix),
		\begin{align*}
...
\begin{prob*}
	...
	\begin{sln*}\textbf{3.}
		...
		\begin{enumerate}
			...
			\item Let $\W \in \mathfrak{Lat}(\lag)$ be given. Claim: $\lag\left[\lag^{-1}[\W] \right] \subseteq \lag^{-1}[\W] $. It suffices to show that:
			\begin{align*}
			\lag\left[ \lag^{-1}[\W] \right] \stackrel{(1)}\subseteq \W \stackrel{(2)}\subseteq \lag^{-1}\left[ \lag[\W] \right] \stackrel{(3)}\subseteq \lag^{-1}[\W],
			\end{align*} 
			where relation $(3)$ comes from the choice of $\W$ such that $\lag[\W] \subseteq \W$.\\
			
			(1) follows from the definition of the pre-image of $\W$ under $\lag : \V \to \W$:
			\begin{align*}
			\lag^{-1}[\W] = \left\{ v \in \V \vert \lag(v) \in \W \right\}.
			\end{align*}
			This implies that $\lag\left[\lag^{-1}[\W]\right] \subseteq \W$.\\
