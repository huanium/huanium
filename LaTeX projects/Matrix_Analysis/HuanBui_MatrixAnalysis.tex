...
\begin{prob*}\textbf{1.}
	...
	\begin{sln*}\textbf{1.}
		...
		\begin{itemize}
			...
			Because $n_0$ is minimal, $\mathbf{S}$ cannot be obtained from a union of less than $n_0$ of the $\W_i$'s. Therefore, for any $i$, there exists $w_i \in \W_i$ such that $w_i \notin \bigcup_{j\neq i}^{n_0}\W_j$; i.e., each $\W_i$ contains an element $w_i$ that does not belong to any other $\W_j$'s.\\
			
			Take $w_1 \in \W_1$ and $w_2\in\W_2\setminus \W_1$ such that $w_1 \notin \bigcup_{j\neq 1}^{n_0}\W_j$. It follows that $w_1, w_2 \in \mathbf{S}$ because $\W_1, \W_2 \subset \mathbf{S}$. And since $\mathbf{S}$ is a subspace, $w_1 + w_2 \in \mathbf{S}$ by closure under addition. Let a collection of the $n_0$ linear combinations of $w_1$ and $w_2$, defined as
			\begin{align*}
			T = \{ m w_1 + w_2 \,\vert\, m = 1,2,\dots,n_0 \}.
			\end{align*}
			Consider a typical element, $m w_1 + w_2 \in \mathbf{S}$. Suppose $mw_1 + w_2 \in \W_1$. By closure under addition, $$(mw_1 + w_2) - mw_1 = w_2\in \W_1$$ for any $m=1,2,\dots,n_0$. But this contradicts the choice of $w_2$. Therefore, $mw_1 + w_2 \notin \W_1$, for any $m$. \\
			...
			\end{align*}
			But since $m_a \neq m_b$, $w_1 \in \W_k$. This contradicts our initial choice of $w_1$. Therefore, by contraposition, the implication $1. \implies 2.$ must hold.
...
\begin{prob*}\textbf{2.}
	...
	\begin{sln*}\textbf{2.}
		...
		\end{pmatrix}\right\},
		\end{align*}
		where $x,y,\in \mathbb{C} $, has two elements. Next, consider $\mathbb{C}^3$. Since $\W$ and $\mathbb{C}^3$ are subspaces and any element of $\W$ is contained in $\mathbb{C}^3$ by construction, $\W$ is a subspace of $\mathbb{C}^3$.\\
		...
		\end{align*}
		So $3 \leq \dim\left( \mathcal{A}^{-1}[\W]  \right)$. But since the dimension of any image of $\mathcal{A}$ cannot exceed 3 (because $\mathcal{A} : \mathbb{C}^3 \to \mathbb{C}^3$ ),
		\begin{align*}
		3 \leq \dim\left( \mathcal{A}^{-1}[\W]  \right) \leq 3,
		\end{align*}
		which implies $\dim\left( \mathcal{A}^{-1}[\W]  \right) = 3 = \dim(\V)$.\\
		
		\noindent Consider $v\in \mathcal{A}^{-1}[\W] $. $v$ can have the form $(x\,\,y\,\,z)^\top$, $x,y,z\in \mathbb{C}$, so $v\in\mathbb{C}^3$. This implies $\mathcal{A}^{-1}[\W] \subseteq \mathbb{C}^3$. But since their dimensions are both 3, $\mathcal{A}^{-1}[\W] = \mathbb{C}^3$. It follows that
		\begin{align*}
		...
		\end{pmatrix}.
		\end{align*}
		First, $\W$ is invariant under $\mathcal{A}$ because take any $(x\,\,\,y\,\,\,0)^\top \in \W$, $\mathcal{A}(x\,\,\,y\,\,\,0)^\top = (x\,\,\,0\,\,\,0)^\top \in \W$, which implies $\mathcal{A}[\W] \subseteq [\W]$ $(a)$. \\
		
		\noindent Second, it is easy to see that the pre-image of $\W$ under $\mathcal{A}$ is $\mathbb{C}^3$, since take any $(x\,\,\,y\,\,\,z)^\top \in \mathbb{C}^3$, we have $\mathcal{A}(x\,\,\,y\,\,\,z)^\top = (x\,\,\,0\,\,\,0)^\top \in \W$. But observe that $\Im(\mathcal{A})$ is proper subset of $\W$, because any $(x\,\,\,y\,\,\,0)^\top \in \W$ with $y\neq 0$ is not contained in $\Im(\mathcal{A})$. This implies $\mathcal{A}\left[\mathcal{A}^{-1}[\W]\right] \subsetneq \W$  $(b)$.\\ 
		
		\noindent Lastly, as we have pointed out, $\dim\left(A^{-1}\left[\mathcal{A}[\W]\right]\right) = 3$ and $A^{-1}\left[\mathcal{A}[\W]\right] \prec \mathbb{C}^3$, so $A^{-1}\left[\mathcal{A}[\W]\right] = \mathbb{C}^3 \supsetneq \W$ $(c)$.\\
...
\begin{prob*}\textbf{4. Cyclic invariant subspaces}
	...
	\begin{sln*}\textbf{4.}
		...
		\begin{enumerate}
			...
			&= \sum^k_{i=0}b_i\lag^i(v_0) + \sum^k_{i=0}c_i\lag^j(v_0)\,\,\,\, \text{with }c_i = 0 \text{ for all } i > l\\
			...
			\item Let $P(\lag,v_0)$ be given. It suffices to show that $\lag(P(\lag, v_0)) \in \mathbf{P}(\lag,v_0)$, for all $P(\lag,v_0)$. $$ \lag(P(\lag,v_0)) = \lag\left( \sum_{i=0}^k a_i\lag^i(v_0) \right) = \sum_{i=0}^k a_i \lag^{i+1}(v_0).$$ Define coefficients $b_{j} = a_{i}$ where $j=i+1$ and $b_0 = 0$, then $$ \lag(P(\lag,v_0)) = \sum_{j=1}^{k+1} b_j\lag^j(v_0) = \sum_{j=0}^{k+1} b_j\lag^j(v_0) \in \mathbf{P}(\lag,v_0). $$
		...
		\begin{enumerate}
			...
			\item $(\impliedby):$ Let $\W = \mathbf{P_1} \cup \mathbf{P_2} \cup \dots \cup \mathbf{P_n}$ be a subspace of $\V$, where $\mathbf{P_i} \equiv \mathbf{P}(\lag,v_i)$ and $v_i$ is an arbitrary element of $\V$. By problem 1, because $\mathbf{P_1} \cup \mathbf{P_2} \cup \dots \cup \mathbf{P_n}$ is a subspace of $\V$, a $\mathbf{P_i}$, $1\leq i \leq n$, contains all the other $\mathbf{P_j}$'s. Without loss of generality, assume such a $\mathbf{P_i}$ is $\mathbf{P_1}$. It follows that $\W = \mathbf{P_1}$. By part 2. of this problem, we know that $\mathbf{P_1}$ is invariant under $\lag$. Therefore, $\W$ is invariant under $\lag$.
...
\begin{prob*} \textbf{5. Invariant subspaces of commuting operators:} Suppose that linear functions $\lag, \mathcal{M} \in \mathfrak{L}(\V)$ commute; i.e.,
	...
	\begin{sln*}\textbf{5.}
		...
		\begin{enumerate}
			...
			Consider $\J = \xpan\{(1\,\,\,1)^\top\}$. $\J \in \mathfrak{Lat}(\mathcal{A})$ since $\mathcal{A}[\J] = \{ \mathbf{0}_\V\} \subseteq \J$. However, consider $j = (1\,\,\,1)^\top \in \J$, we have
			\begin{align*}
	...
	\begin{sln*}\textbf{6.}
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				Assume that $ \lag^{k+1}[\W] \subseteq \lag^k[\W] $ for any $k\geq 0$. The base case $k=0$ is true because $\W$ is an invariant subspace of $\lag$. The inductive case is also true because $$ \lag^{k+2}[\W] = \lag[\lag^{k+1}[\W]] \subseteq \lag[\lag^k[\W]] = \lag^{k+1}[\W],$$ where the second relation follows from assumption. By principle of induction, $\dots \subseteq \lag^3[\W] \subseteq \lag^2[\W]\subseteq \lag [\W] \subseteq \W$.\\
				
				\item \underline{To show}: $\W \subseteq \lag^{-1}[\W] \subseteq \lag^{-2}[\W] \subseteq \lag^{-3}[\W]\subseteq\dots$.\\
				
				Assume that $\lag^{-k}[\W] \subseteq \lag^{-k-1}[\W] $ for all $k\geq 0$. The base case where $k=0$ is true: Given $w\in\W\subseteq \V$, $\lag(w) \in \W$ because $\W$ is invariant under $\lag$. But since $\lag^{-1}[\W] = \{ v\in\V \vert \lag(v)\in \W\}$, $w \in \lag^{-1}[\W]$. Therefore, $\W \in \lag^{-1}[\W]$ (i).\\
			...
			\begin{enumerate}
				...
				Because $\dim(\lag^{-k}[\W]) \leq \dim(\V)$ for any $k\geq 0$, there exists an $\lag^{-h}[\W]$, $h\geq 0$, that is an invariant subspace in the chain with the highest dimension where $h$ is maximal. It immediately follows that $\dim(\lag^{-(h+1)}[\W]) = \dim(\lag^{-(h)}[\W])$ is maximal. But because $\lag^{-h}[\W] \subseteq \lag^{-(h+1)}[\W]$, $\lag^{-h}[\W] = \lag^{-(h+1)}[\W]$. As a result, $\lag^{-h}[\W] = \lag^{-j}[\W]$ for any $j\geq h$, which follows from (b). Therefore, the chain in part 1. stabilizes in the ``inclusion'' direction.  \\
				...
				This implies $\dim(\lag^{-(h+j)}[\W]) < \dim(\lag^{-h}[\W]) \leq \dim(\V)$. Suppose $\dim(\lag^{-h}[\W]) = \dim(\V)$, i.e., maximal, then $$\dim(\lag^{-(h+j)}[\W]) \leq \dim(\V) - 1.$$ By assuming equality is met and consider the next proper inclusion in the chain, a similar argument shows that the invariant subspace of interest will have at most $\dim(\V) - 2$ dimensions. It follows that, at the $\dim(\V)^{\text{th}}$ proper inclusion, the considered invariant subspace has at most $\dim(\V) - \dim(\V) = 0$ dimensions; i.e. the subspace is $\{\mathbf{0}_\V\}$. At this point, no further proper inclusion is possible. Therefore, the chain in part 1. has at most $\dim(\V)$ proper inclusion.\\
			\end{enumerate}
