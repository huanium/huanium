...
\begin{prob*}\textbf{1.}
	...
	\begin{sln*}\textbf{1.}
		...
		\begin{itemize}
			...
			Take non-null $w_1 \in \W_1$ and $w_2\in\W_2\setminus \W_1$ such that $w_1 \notin \bigcup_{j\neq 1}^{n_0}\W_j$. It follows that $w_1, w_2 \in \mathbf{S}$ because $\W_1, \W_2 \subset \mathbf{S}$. And since $\mathbf{S}$ is a subspace, $w_1 + w_2 \in \mathbf{S} = \W_1 \cup \W_2 \cup \dots \cup \W_{n_0}$ by closure under addition. Let a collection of the the $n_0$ linear combinations of $w_1$ and $w_2$ be defined as
			\begin{align*}
			...
			It follows that $T \subset \bigcup_{i=2}^{n_0} \W_i$. By construction, $T$ has $n_0$ elements, while $\bigcup_{i=2}^{n_0} \W_i$ has $(n_0 - 1)$ $\W_i$'s. By the Pigeonhole Principle, a $\W_m$ of $\W_2, \dots, \W_{n_0}$ contains $mw_1 + w_2$ for two distinct values of $m$. Let these values be $m_a$ and $m_b$. By closure under addition, we have
			\begin{align*}
			(m_a w_1 + w_2) - (m_b w_1 + w_2) = (m_a - m_b)w_1 \in \W_m.
			\end{align*}
			Since $m_a \neq m_b$, $w_1 \in \W_m$. But this contradicts our initial choice of $w_1$. Therefore, by contraposition, the implication $1. \implies 2.$ must hold.
