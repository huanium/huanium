...
\begin{prob*} 1. Suppose that $\V_1, \V_2\,\dots,\V_{315}$ are subspaces of a vector space $\W$. Argue that the following claims are equivalent. 
	...
	\begin{sln*}
		...
		\begin{itemize}
			...
			\item ($2\implies 1$) Let $\hat{h} \in \V_1 + \V_2 + \dots + \V_{315}$ be given such that
			\begin{align}
			\hat{h} = x_1 + x_2 + \dots + x_{315} = y_1 + y_2 + \dots + y_{315} ,
			\end{align}
			where $x_i, y_i \in \V_i$. It suffices to show $x_i = y_i$, i.e, any representation of $\hat{h}$ as a combination of $x_i$ $\in \V_i$ is unique. Consider 
			\begin{align*}
			\mathbf{0}_\W = \hat{h} - \hat{h} = (x_1 - y_1) + (x_2 - y_2) + \dots + (x_{315} - y_{315}).
			\end{align*}
			By the second statement, $x_i - y_i = \mathbf{0}_\W$ for all $i$, i.e., $x_i = y_i\,\,\forall i$. Therefore, any representation of $\hat{h}$ as Eq. (3) is unique, as desired by the first statement. 
			\item ($3\implies 2$) Suppose that the second statement is false. Without loss of generality, let $x_1 \in \V_1$, $x_1 \neq \mathbf{0}_\W$ be given and 
			\begin{align*}
			x_1 + x_2 + \dots + x_{315} = \mathbf{0}_\W.
			\end{align*}
			It immediately follows that
			\begin{align*}
			-(x_2 + x_3+\dots + x_{315}) = x_1 \neq \mathbf{0}_\W,
			\end{align*}
			which implies the third statement is also incorrect. By contraposition, $(3)$ implies $(2)$.
			
			
			\item ($2\implies 3$) Suppose that the third statement is false. Let $x_1 = -x_2 \neq \mathbf{0}_\W$, $x_1 \in \V_1$, $x_2\in \V_2$ be given. Then with $x_j = \mathbf{0}_\W$ for all $j=3,4,\dots,315$,
			\begin{align*}
			x_1 + x_2 + x_3 + \dots + x_{315} &= x_1 -x_1 + x_3 + \dots + x_{315}\\ &= x_3 + \dots + x_{315}\\ &=\mathbf{0}_\W.
			\end{align*}
			Since $x_1 = -x_2 \neq \mathbf{0}_\W$, there is a contradiction. Therefore the second statement no longer holds. By contraposition, it implies the third statement. 
			\item ($3\iff 4$) This is evidently true since ``sum of the elements of the preceding $\V_j$'' is a subset of ``sums of the elements of the other $\V_j$'s.'' So, the fourth statement is true. Conversely, if no non-null element of $\V_i$ can be expressed as a sum of the elements of the preceding $\V_j$'s, then the third statement is also true. (Imagine going through an ordered list, checking for a counter example, and not succeeding). 
		\end{itemize}
	\end{sln*}
\end{prob*}
...
\begin{prob*} 2. \textbf{Sub-sums of direct sums are direct: }\\
	...
	\begin{sln*}
		$\,$
		\begin{enumerate}
			\item By the previous problem, because $\V_1+\V_2+\dots+\V_{315}$ is direct, for any index $i$, no non-null element of $\V_i$ can be expressed as a sum of the elements of the other $\V_j$'s. Since $\Z_i$ is a subspace of $\V_i$, no $z_i\in \Z_i \prec \V_i$, $z_i \neq \mathbf{0}_\W$, can be expressed as a sum of the elements of the other $\Z_j$'s $\prec \V_j$'s. Hence, the subspace sum $\Z_1 + \Z_2 + \dots \Z_{315}$ is also direct.
			\item This is true by (1). Because each $\V_j$ is a subspace of itself, the subspace sum of distinct $\V_j$'s is direct. Therefore, the sum $\V_2 + \V_5 + \V_7 + \V_{12}$ is direct.
		\end{enumerate}
	\end{sln*}
\end{prob*}
...
\begin{prob*} 3. \textbf{Associativity of directness of subspace sums:} Suppose that 
	...
	\begin{sln*}
		...
		\begin{enumerate}
			...
			\end{align}
			Because vector space addition is associative, equality (4) is equivalent to the subspace sum in the first statement being direct. Hence, (2) implies (1). 
		\end{enumerate}
	\end{sln*}
\end{prob*}
...
\begin{prob*} 4. \textbf{Direct sums preserve linear independence:} Suppose that $\U,\V,\W$ are subspaces of a vector space $\Z$, and the sum $\U,\V,\W$ is direct. 
	...
	\begin{sln*}
		$\,$
		\begin{enumerate}
			\item It suffices to consider a linearly independent $U_1\in \U$ and subspace $\V$. Since $\U +\V +\W$ is a direct sum, by the previous problem, $\U + \V$ is a direct sum. Therefore, $U_1\neq \mathbf{0}_\Z$ cannot be expressed as a sum of the elements of $\V$, i.e., $U_1$ is linearly independent of all $V_i$'s $\in \V$. The same is true with regards to $U_1$ and all $W_i$'s $\in \W$.\\
			
			\noindent It follows that if $U_1,U_2,\dots,U_{13}$ is a linearly independent list in $\U$, $V_1,V_2,\dots,V_{6}$ is a linearly independent list in $\V$, and $W_1,W_2,\dots,W_{134}$ is a linearly independent list in $\W$, then the list
			\begin{align*}
			U_1,U_2,\dots,U_{13},V_1,V_2,\dots,V_{6},W_1,W_2,\dots,W_{134}
			\end{align*}
			is linearly independent.
...
\begin{prob*} 5. Suppose that $x_1, x_2,\dots,x_{14}$ are non-null elements of a linear space $\W$. 
	...
	\begin{sln*}
		...
		\begin{enumerate}
			\item 
			\begin{enumerate}
				\item ($a\implies b$) If $x_1, x_2,\dots,x_{14}$ are linearly independent, then any $x_i$ is neither a null element nor can be expressed as a linear combination of the other $x_j$'s. Therefore, no non-null $x_i \in \xpan(x_i)$ can be written as a sum of other $x_j$'s $\in \xpan(x_j)$. This is equivalent to 
				\begin{align*}
				\xpan(x_1) + \xpan(x_1) + \dots + \xpan(x_{14})
				\end{align*}
				being direct, by Problem 1. 
			...
			\end{enumerate}
			\item Because $x_i$ is a basis of $\xpan(x_i)$, the concatenated list $x_1,x_2,\dots,x_{14}$ is a basis of $\xpan(x_1)\oplus\xpan(x_2)\oplus\dots\oplus\xpan(x_{14})$, by Problem 4$^\dagger$.
			\begin{enumerate}
				\item ($a\implies b$) Let $w\in\xpan(x_1)\oplus\xpan(x_2)\oplus\dots\oplus\xpan(x_{14})$ be given, then by ($\dagger$), $w$ can be expressed as a combination of the $x_i$'s, where $x_i\in\xpan(x_i)$. But this is equivalent to $w\in\W$ because $x_1,x_2,\dots,x_{14}$ is a basis of $\W$. Therefore, $\W = \xpan(x_1)\oplus\xpan(x_2)\oplus\dots\oplus\xpan(x_{14})$ by $(\dagger)$.
				\item ($b\implies a$) It follows from ($\dagger$) that if $\W = \xpan(x_1) \oplus \xpan(x_2) \oplus \dots \oplus \xpan(x_{14})$, then $x_1, x_2,\dots,x_{14}$  is a basis of $\W$.
			\end{enumerate}
		\end{enumerate}
	\end{sln*}
\end{prob*}
