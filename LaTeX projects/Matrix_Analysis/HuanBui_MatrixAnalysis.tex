...
\begin{prob*}\textbf{4. Cyclic invariant subspaces}
	...
	\begin{sln*}\textbf{4.}
		...
		\begin{enumerate}
			...
			It is also clear that $\mathbf{P}(\lag,v_0)$ is closed under scalar multiplication because given $\mu\in \mathbb{C}$, $$\mu P_1(\lag,v_0) = \mu\sum_{i=0}^kb_i\lag^i(v_0) = \sum_{i=0}^k\mu b_i\lag^i(v_0) = \sum_{i=0}^ke_i\lag^i(v_0) \in \mathbf{P}(\lag,v_0),$$ where $e_i = \mu b_i \in \mathbb{C}$ (iii).\\
			
			By (i), (ii), and (iii), $\mathbf{P}(\lag,v_0)$ is a subspace of $\V$. 
			...
			\item If $v_0$ is an eigenvector of $\lag$, then $\lag^i(v_0) = \lambda^i v_0$, where $\lambda^i \in \mathbb{C}$ is the $v_0$-eigenvalue raised to the $i^{\text{th}}$ power. It follows that a typical element of $\mathbf{P}(\lag,v_0)$ is $$P(\lambda,v_0) = \sum_{i=0}^k a_i \lambda^i v_0 =  v_0 \left(\sum_{i=0}^k a_i\lambda^i\right) = \mu v_0,$$
	...
	\begin{sln*}\textbf{6.}
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				From (i) and (ii), $ \dots \subseteq \lag^3[\W] \subseteq \lag^2[\W]\subseteq \lag [\W] \subseteq \W \subseteq \lag^{-1}[\W] \subseteq \lag^{-2}[\W] \subseteq \lag^{-3}[\W]\subseteq\dots $
			...
			\begin{enumerate}
				...
				\item Let $\lag^h[\W]$ be an invariant subspace in the chain with the highest dimension where $h$ is minimal. It follows from part 1. that
				\begin{align*}
				correct
				\end{align*}
				
				For every \textit{proper} inclusion, there is a dimension lost. Without loss of generality, consider $\lag^{(k+1)}[\W] \subsetneq \lag^k[\W] \subsetneq \lag^{(k-1)}[\W]$ for some integer $k$. It follows that $\dim(\lag^{(k+1)}[\W]) < \dim(\lag^{k}[\W]) < \dim(\lag^{(k-1)}[\W])$. \\
			...
			\item	
			Let $\lag^{(k+1)}[\W] \subseteq \mathbf{M} \subseteq \lag^k[\W]$ be given. $\mathbf{M}$ is a subspace.  
