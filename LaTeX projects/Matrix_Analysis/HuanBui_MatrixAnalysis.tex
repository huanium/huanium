...
\begin{prob*}\textbf{2.}
	...
	\begin{sln*}\textbf{2.}
		...
		\end{align*}
		Since $\dim(\W) = 2$ and $\mathcal{A}$ is a $3\times 3$ matrix, it is required that $\dim\left( \mathcal{A}\left[ \mathcal{A}^{-1}[\W] \right]   \right) = 1$ and $\dim\left(\mathcal{A}^{-1}\left[ \mathcal{A}[\W] \right]\right) = 3$. 
		
%		\noindent It is true that 
%		\begin{align*}
%		A\left[A^{-1}[\W] \right] \subseteq \W.
%		\end{align*}
%		\begin{proof}
%			Let $w \in A\left[A^{-1}[\W] \right]$ be given. By this choice, there exists some $v \in A^{-1}[\W]$ such that $w = A(v)$. Because $v\in A^{-1}[\W]$ is the pre-image of $\W$ under $\mathcal{A}$; i.e.,
%			\begin{align*}
%			A^{-1}[\W] = \{ \nu \in \V \vert \mathcal{A}(\nu)\in \W \},
%			\end{align*}
%			it must be true that $\mathcal{A}(v) \in \W$. Therefore, $w \in \W$, which implies
%			\begin{align*}
%			A\left[A^{-1}[\W]\right] \subseteq \W.
%			\end{align*}
%		\end{proof}
%		Equality occurs when given $w\in \W$, we also have $w \in A\left[ A^{-1}[\W] \right]$, which implies the restricted map 
	\end{sln*}
