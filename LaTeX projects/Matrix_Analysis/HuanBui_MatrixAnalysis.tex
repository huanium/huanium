...
\begin{prob*}\textbf{1.}
	...
	\begin{sln*}\textbf{1.}
		$\,$
		\begin{itemize}
			\item $[1. \implies 2.]:$ Assume the hypothesis. Suppose $2.$ is false. Then there exists a minimal $n_0$ such that the implication $1. \implies 2.$ fails to hold. If $n=1$, then the implication is true because $\W_1$ contains itself. Therefore, in order for this implication to fail, $n_0 \geq 2$.\\
			
			\noindent Let $\W_1, \W_2\,\dots,\W_{n_0}$ be a collection of subspaces of $\V$ such that 
			\begin{align*}
			\mathbf{S} = \bigcup_{i=1}^{n_0}\W_i
			\end{align*}
			is a subspace of $\V$ but none of the $\W_i$'s contains all the others. Because $n_0$ is minimal, $\mathbf{S}$ cannot be obtained from a union of less than $n_0$ of the $\W_i$'s. Therefore, for any $i$, there exists $w_i \in \W_i$ such that $w_i \notin \bigcup_{j\neq i}^{n_0}\W_j$; i.e., each $\W_i$ contains an element $w_i$ that does not belong to any other $\W_j$'s.\\
			
			Take non-null $w_1 \in \W_1$ and $w_2\in\W_2\setminus \W_1$ such that $w_1 \notin \bigcup_{j\neq 1}^{n_0}\W_j$. It follows that $w_1, w_2 \in \mathbf{S}$ because $\W_1, \W_2 \subset \mathbf{S}$. And since $\mathbf{S}$ is a subspace, $w_1 + w_2 \in \mathbf{S} = \W_1 \cup \W_2 \cup \dots \cup \W_{n_0}$ by closure under addition. Let a collection of the $n_0$ linear combinations of $w_1$ and $w_2$ be defined as
			\begin{align*}
			T = \{ m w_1 + w_2 \,\vert\, m = 1,2,\dots,n_0 \}.
			\end{align*}
			Consider a typical element, $m w_1 + w_2 \in \mathbf{S}$. To show that $m w_1 + w_2 \in \bigcup_{i=2}^{n_0} \W_i$, it suffices to show $m w_1 + w_2$ cannot be contained in $\W_1$.\\

			Suppose that $mw_1 + w_2 \in \W_1$. By closure under addition, it must also be true that $(mw_1 + w_2) - mw_1 = w_2\in \W_1$. But this contradicts the choice of $w_2$. Therefore, $mw_1 + w_2 \notin \W_1$. \\
			
			It follows that $T \subset \bigcup_{i=2}^{n_0} \W_i$. By construction, $T$ has $n_0$ elements, while $\bigcup_{i=2}^{n_0} \W_i$ has $(n_0 - 1)$ $\W_i$'s. By the Pigeonhole Principle, a $\W_k$ of $\W_2, \dots, \W_{n_0}$ contains $mw_1 + w_2$ for two distinct values of $m$. Let these values be $m_a$ and $m_b$. By closure under addition, we have
			\begin{align*}
			(m_a w_1 + w_2) - (m_b w_1 + w_2) = (m_a - m_b)w_1 \in \W_k,
			\end{align*}
			since $m_a \neq m_b$, $w_1 \in \W_k$. But this contradicts our initial choice of $w_1$. Therefore, by contraposition, the implication $1. \implies 2.$ must hold.
			...
			\end{align*}
			Since $\W_1$ is a subspace of $\V$, $\bigcup_{i=1}^{n_0}\W_i$ is also a subspace of $\V$. Therefore $2. \implies 1.$.
		\end{itemize}
	\end{sln*}
...
\begin{prob*}\textbf{2.}
	...
	\begin{sln*}\textbf{2.}
		...
		\end{pmatrix}\right\},
		\end{align*}
		where $x,y,\in \mathbb{C} $, has two elements. Next, we define a vector space $\V$ with $\dim(\V) = 3$
		\begin{align*}
		\V = \left\{ \begin{pmatrix}
		x\\y\\z
		\end{pmatrix}\bigg\vert \,x,y,z\in \mathbb{C} \right\}.
		\end{align*}
		Since $\W$ and $\V$ are subspaces and any element of $\W$ is contained in $\V$ by construction, $\W$ is a subspace of $\V$.\\
		...
		\end{align*}
		Since $\dim(\W) = 2$ and $\mathcal{A}$ is a $3\times 3$ matrix (i.e., neither the dimension of $\ker(\mathcal{A})$ nor that of $\Im(A)$ can exceed 3), it is required that $\dim\left( \mathcal{A}\left[ \mathcal{A}^{-1}[\W] \right]   \right) \leq 1$ and $\dim\left(\mathcal{A}^{-1}\left[ \mathcal{A}[\W] \right]\right) = 3$. \\
		...
		\begin{align*}
		\dim\left( \mathcal{A}^{-1}\left[ \mathcal{A}[\W] \right]  \right) \leq \dim\left( \mathcal{A}^{-1}[\W]  \right).
		\end{align*}
		So $3 \leq \dim\left( \mathcal{A}^{-1}[\W]  \right)$. But since the dimension of any image of $\mathcal{A}$ cannot exceed 3 (because $\mathcal{A}$ is a $3\times 3$ matrix),
		\begin{align*}
		3 \leq \dim\left( \mathcal{A}^{-1}[\W]  \right) \leq 3,
		\end{align*}
		which implies $\dim\left( \mathcal{A}^{-1}[\W]  \right) = 3 = \dim(\V)$. Consider $v\in \mathcal{A}^{-1}[\W] $, i.e., $v$ has the form $(x,y,z)^\top$ where $x,y,z\in \mathbb{C}$. This implies $v\in \V$, i.e., $\mathcal{A}^{-1}[\W] \subseteq \V$. But since their dimensions are both 3, $\mathcal{A}^{-1}[\W] = \V$. It follows that
		\begin{align*}
		\mathcal{A}\left[ \mathcal{A}^{-1}[\W] \right] = \mathcal{A}[\V] \supseteq \mathcal{A}[\W],
		\end{align*}
		where the second relation comes from the fact that $\W$ is a subspace of $\V$. Hence
		\begin{align*}
		1 \geq \dim\left( \mathcal{A}\left[ \mathcal{A}^{-1}[\W] \right] \right) \geq \dim\left( \mathcal{A}[\W] \right),
		\end{align*}
		which implies $\dim(\mathcal{A}[\W]) = 0$ or $1$. Such a matrix $\mathcal{A}$ can be
		\begin{align*}
		...
		\end{pmatrix}.
		\end{align*}
		First, $\W$ is invariant under $\mathcal{A}$ because take any $(x\,\,\,y\,\,\,0)^\top \in \W$, $\mathcal{A}(x\,\,\,y\,\,\,0)^\top = (x\,\,\,0\,\,\,0)^\top \in \W$, which implies $\mathcal{A}[\W] \subseteq [\W]$ ($(a)$). Second, it is also easy to see that the pre-image of $\W$ under $\mathcal{A}$ is $\V$, since take any $(x\,\,\,y\,\,\,z)^\top \in \V$, we have $\mathcal{A}(x\,\,\,y\,\,\,z)^\top = (x\,\,\,0\,\,\,0)^\top \in \W$. But observe that $\Im(\mathcal{A})$ is proper subset of $\W$, because any $(x\,\,\,y\,\,\,0)^\top \in \W$ with $y\neq 0$ is not contained in $\Im(\mathcal{A})$ ($(b)$). This implies $\mathcal{A}\left[\mathcal{A}^{-1}[\W]\right] \subsetneq \W$. Lastly, as we pointed out above, $\dim\left(A^{-1}\left[\mathcal{A}[\W]\right]\right) = 3$ and $A^{-1}\left[\mathcal{A}[\W]\right] \prec \V$, so $A^{-1}\left[\mathcal{A}[\W]\right] = \V \supsetneq \W$ $(c)$.\\
		
		From $(a),(b),(c)$, we see that this $3\times 3$ matrix $\mathcal{A}$ works as desired.   
	\end{sln*}
...
\begin{prob*}
	...
	\begin{sln*}\textbf{3.}
		...
		\begin{enumerate}
			...
			\item (MORE CARE NEEDED) Let subspaces $\V,\W \in \mathfrak{Lat}(\lag)$ be given. $\mathfrak{Lat}(\lag)$ is closed under subspace sums if $\V + \W \in \mathfrak{Lat}(\lag)$. Consider $\lag[\V + \W] = \lag[\V] + \lag[\W]$. Since $\lag[\V] \subseteq \V$ and $\lag[\W] \subseteq \W$ by the definitions of $\V$ and $\W$, it must be true that $\lag[\V] + \lag[\W] \subseteq \V + \W$. Therefore, $\lag[\V + \W] \subseteq \V + \W$, which implies $\V + \W \in \mathfrak{Lat}(\lag)$. This completes the argument.
			...
			\item Let $\W \in \mathfrak{Lat}(\lag)$ be given. Claim: $\lag\left[\lag^{-1}[\W] \right] \subseteq \lag^{-1}[\W] $. It follows from the definition of the pre-image of $\W$ under $\lag : \V \to \W$ that $\lag^{-1}[\W] = \left\{ v \in \V \vert \lag(v) \in \W \right\}$. This implies that $\lag\left[\lag^{-1}[\W]\right] \subseteq \W$ (i).\\
			
			Next, consider $w\in\W \subseteq \V$. By definition, $\lag^{-1}\left[\lag[\W] \right] = \{ v\in \V \vert \lag(v) \in \lag[\W]\},$ which implies that $w$ is such a $v$, because $\lag(w) \in \lag[\W]$. Therefore, $\W \subseteq \lag^{-1}\left[\lag[\W]\right]$ (ii).\\   
			
			Moreover, because $\W \in \mathfrak{Lat}(\lag)$, $\lag[\W] \subseteq \W$ (iii).\\ 
			
			From (i), (ii), and (iii) we have
			\begin{align*}
			\lag\left[ \lag^{-1}[\W] \right] \stackrel{(1)}\subseteq \W \stackrel{(2)}\subseteq \lag^{-1}\left[ \lag[\W] \right] \stackrel{(3)}\subseteq \lag^{-1}[\W].
			\end{align*} 
...
\begin{prob*}\textbf{4. Cyclic invariant subspaces}
	...
	\begin{sln*}\textbf{4.}
		...
		\begin{enumerate}
			...
			Consider $P_1(\lag, v_0) = \sum^k_{i=0}b_i\lag^i(v_0)$ and $P_2(\lag,v_0) = \sum^l_{j=0}c_j\lag^j(v_0)$, with $b_i, c_j \in \mathbb{C}$ for all $i,j$. Without loss of generality, assume that $k\geq l$. It is clear that $\mathbf{P}(\lag,v_0)$ is closed under addition because, 
			\begin{align*}
			P_1(\lag,v_0) + P_2(\lag,v_0) &= \sum^k_{i=0}b_i\lag^i(v_0) + \sum^l_{j=0}c_j\lag^j(v_0)\\
			&= \sum^k_{i=0}b_i\lag^i(v_0) + \sum^k_{i=0}c_i\lag^j(v_0)\,\,\,\,\, c_i = 0 \text{ if } i > l\\
			...
			\item Let $P(\lag,v_0)$ be given. It suffices to show that $\lag(P(\lag, v_0)) \in \mathbf{P}(\lag,v_0)$. $$ \lag(P(\lag,v_0)) = \lag\left( \sum_{i=0}^k a_i\lag^i(v_0) \right) = \sum_{i=0}^k a_i \lag^{i+1}(v_0).$$ Define coefficients $b_{j} = a_{i}$ where $j=i+1$ and $b_0 = 0$, then $$ \lag(P(\lag,v_0)) = \sum_{j=1}^{k+1} b_j\lag^j(v_0) = \sum_{j=0}^{k+1} b_j\lag^j(v_0) =  \sum_{j=0}^{k+1} b_j\lag^j(v_0) \in \mathbf{P}(\lag,v_0). $$
			Therefore, $\lag\left[\mathbf{P}(\lag,v_0) \right] \subseteq \mathbf{P}(\lag,v_0)$; i.e., $\mathbf{P}(\lag,v_0)$ is invariant under $\lag$. \\
			
			
			
			\item  Let $\mathbf{P}(\lag,v_0)$ be given. $\mathbf{P}(\lag,v_0)$ is an invariant under $\lag$ that contains $v_0$ by definition. Suppose $\mathbf{Q}(\lag,v_0)$ is some invariant subspace under $\lag$ that also contains $v_0$. It suffices to show $\mathbf{P}(\lag,v_0) \subseteq \mathbf{Q}(\lag,v_0)$ for any such $\mathbf{Q}(\lag,v_0)$. \\
			...
			\item $(\implies):$ Let a $\mathbf{P}(\lag,v_0)$ be given such that $\dim(\mathbf{P}(\lag,v_0)) = 1$. Consider the subspace $\xpan(v_0)$. We know that $\dim(\xpan(v_0)) = 1$. Consider $v \in \xpan(v_0)$. Then $v$ can be expressed as $v = av_0$ where $a\in \mathbb{C}$. It follows immediately that $v\in \mathbf{P}(\lag,v_0)$. Therefore, $\xpan(v_0) \subseteq \mathbf{P}(\lag,v_0)$. But because $\dim(\mathbf{P}(\lag,v_0)) = 1 = \dim(\xpan(v_0))$, it follows that $\mathbf{P}(\lag,v_0) = \xpan(v_0)$; i.e., all elements of $\mathbf{P}(\lag,v_0)$ are some scalar multiples of $v_0$.\\
			
			Now, it suffices to show $\lag(v_0) = \lambda v_0$, where $\lambda \in \mathbb{C}$. Consider $P(\lag, v_0) = \lag^0(v_0) + \lag(v_0) \in \mathbf{P}(\lag,v_0) = \xpan(v_0)$, $, v_0 \neq \mathbf{0}_\V$. By closure under addition, $ (\lag^0(v_0) + \lag(v_0)) - \lag^0(v_0) = \lag(v_0) \in \xpan(v_0)$. This implies $\lag(v_0) = \lambda v_0$ for some $\lambda \in \mathbb{C}$; i.e., $v_0$ is an eigenvector of $\lag$.\\
				
				
			\item $(\impliedby):$ If $v_0$ is an eigenvector of $\lag$, then $\lag^i(v_0) = \lambda^i v_0$, where $\lambda^i \in \mathbb{C}$ is the $v_0$-eigenvalue raised to the $i^{\text{th}}$ power. It follows that a typical element of $\mathbf{P}(\lag,v_0)$ is $$P(\lambda,v_0) = \sum_{i=0}^k a_i \lambda^i v_0 =  v_0 \left(\sum_{i=0}^k a_i\lambda^i\right) = \mu v_0,$$
		...
		\begin{enumerate}
			...
			\item $(\impliedby):$ Let $\W = \mathbf{P_1} \cup \mathbf{P_2} \cup \dots \cup \mathbf{P_n}$ be a subspace of $\V$, where $\mathbf{P_i} \equiv \mathbf{P}(\lag,v_i)$ and $v_i$ is an arbitrary element of $\V$. By problem 1, because $\mathbf{P_1} \cup \mathbf{P_2} \cup \dots \cup \mathbf{P_n}$ is a subspace of $\V$, a $\mathbf{P_i}$, $1\leq i \leq n$, contains all the other $\mathbf{P_i}$'s. Without loss of generality, assume such a $\mathbf{P_i}$ is $\mathbf{P_1}$. It follows that $\W = \mathbf{P_1}$. By part 2. of this problem, we know that $\mathbf{P_1}$ is invariant under $\lag$. Therefore, $\W$ is invariant under $\lag$.
...
\begin{prob*} \textbf{5. Invariant subspaces of commuting operators:} Suppose that linear functions $\lag, \mathcal{M} \in \mathfrak{L}(\V)$ commute; i.e.,
	...
	\begin{sln*}\textbf{5.}
		$\,$
		\begin{enumerate}
			\item Because $\lag \circ \M = \M \circ \lag$, $\lag \circ \M^k = \M^k \circ \lag$ for any non-negative $k$. We can verify this by a short proof by induction. Assume that $\lag \circ \M^k = \M^k \circ \lag$ holds for any non-negative $k$. The base case where $k=0$ is trivially true, for $\lag\circ \M^0 = \lag \circ I = I \circ \lag = \M^0 \circ \lag$. The inductive case is also true because $$ \lag\circ \M^{k+1} = \lag \circ \M^k \circ \M = \M^k\circ\lag\circ\M = \M^k\circ \M \circ \lag = \M^{k+1}\circ \lag.$$ Therefore, by the principle of induction, $\lag \circ \M^k = \M^k \circ \lag$ is indeed true for all $k$.
			
			\begin{enumerate}
				\item \underline{To show}: $\lag\left[ \Im(\M^k)\right] \subseteq \Im(\M^k)$. Since $\lag\circ \M^k = \M^k \circ \lag$,  $\lag\circ \M^k[\V] = \M^k\circ\lag[\V]$; i.e., $\lag[\Im(\M^k)] = \M^k[\Im(\lag)]$. But since $\Im(\lag) \subseteq \V$, $\M^k[\Im(\lag)] \subseteq \M^k[\V] = \Im(\M^k)$. It follows that $\lag[\Im(\M^k)] \subseteq \Im(\M^k)$. So, $\Im(\M^k)$ is an invariant subspace for $\lag$.
				
				\item \underline{To show}: $\lag[\ker(\M^k)] \subseteq \ker(\M^k)$. Since $\lag\circ \M^k = \M^k \circ \lag$, $\lag\circ \M^k[\ker(\M^k)] = \{\mathbf{0}_\V\} = \M^k \circ \lag[\ker(\M^k)]$. It follows that $\lag[\ker(\M^k)] \subseteq \ker(\M^k)$. This completes the argument.
			\end{enumerate}
			
			
			\item Let $\mathbf{E}_\lambda$ the eigenspace of $\M$ associated with eigenvalue $\lambda$ be given. Consider $e \in \mathbf{E}_\lambda$, we have $\lag \circ \M(e) = \lag(\lambda e) = \lambda \lag(e) = \M\circ \lag(e)$. This implies $\lag(e) \in \mathbf{E}_\lambda$. Since this relation holds for any $e\in \mathbf{E}_\lambda$, $\lag[\mathbf{E}_\lambda] \subseteq \mathbf{E}_\lambda$. Therefore, $\mathbf{E}_\lambda$ is an invariant subspace for $\lag$ for any eigenvalue $\lambda$. This proves the claim that \textit{every} eigenspace of $\M$ is an invariant subspace for $\lag$.\\
			...
			Consider $\J = \xpan\{(1\,\,\,1)^\top\}$. $\J \in \mathfrak{Lat}(\mathcal{A})$ since $\mathcal{A}(\J) = \{ \mathbf{0}_\V\} \subseteq \J$ for any $j\in \J$. However, consider $j = (1\,\,\,1)^\top \in \J$, we have
			\begin{align*}
			...
			Observe that we can give a more general example for any $n>1$. Let $\mathcal{A}$ be an $n\times n$ zero matrix, and $\mathcal{B}$ is an $n\times n$ matrix of the form
			\begin{align*}
			\mathcal{B} = \begin{pmatrix}
			1&0&\dots&0\\
			0&\ddots\\
			...
			\end{pmatrix}.
			\end{align*}
			It is also clear that $\mathcal{A}$ and $\mathcal{B}$ commute, since $\mathcal{A}$ is the zero matrix: $\mathcal{AB} = [0]_{n\times n} = \mathcal{BA}$.\\
		
			Consider $j = (1\,\,1\,\,\dots\,\,1)^\top \in \J = \xpan\{ (1\,\,1\,\,\dots\,\,1)^\top \} \subseteq \mathbb{C}^n$. It is clear that $\mathcal{B}(j) = (1\,\,0\,\,\dots\,\,0)^\top \notin \J$. So, $\J \in \mathfrak{Lat}(\mathcal{A})$ but $\J \notin \mathfrak{Lat}(\mathcal{B})$; i.e., $\mathfrak{Lat}(\mathcal{A}) \neq \mathfrak{Lat}(\mathcal{B})$. 
		\end{enumerate}
	\end{sln*}
\end{prob*}
...
\begin{prob*}\textbf{6. Invariant subspace chains:} Suppose that $\lag \in \mathfrak{L}(\V)$ and $\W$ is an invariant subspace of $\lag$. 
	...
	\begin{sln*}\textbf{6.}
		...
		\begin{enumerate}
			\item 
			\begin{enumerate}
				\item \underline{To show}: $\dots \subseteq \lag^3[\W] \subseteq \lag^2[\W]\subseteq \lag [\W] \subseteq \W$. Assume that $ \lag^{k+1}[\W] \subseteq \lag^k[\W] $ for any $k\geq 0$. The base case $k=0$ is true because $\W$ is an invariant subspace of $\lag$. The inductive case is also true because $$ \lag^{k+2}[\W] = \lag[\lag^{k+1}[\W]] \subseteq \lag[\lag^k[\W]] = \lag^{k+1}[\W],$$ where the second relation follows from assumption. By principle of induction, $\dots \subseteq \lag^3[\W] \subseteq \lag^2[\W]\subseteq \lag [\W] \subseteq \W$.\\
				
				\item \underline{To show}: $\W \subseteq \lag^{-1}[\W] \subseteq \lag^{-2}[\W] \subseteq \lag^{-3}[\W]\subseteq\dots$. Assume that $\lag^{-k}[\W] \subseteq \lag^{-k-1}[\W] $ for all $k\geq 0$. The base case where $k=0$ is true: Given $w\in\W\subseteq \V$, $\lag(w) \in \W$ because $\W$ is invariant under $\lag$. But since $\lag^{-1}[\W] = \{ v\in\V \vert \lag(v)\in \W\}$, it follows that $w \in \lag^{-1}[\W]$; i.e., $\W \in \lag^{-1}[\W]$ (i).\\
				
				The inductive case is also true: $$ \lag^{-k-1}[\W] = \lag^{-1}[\lag^{-k}[\W]] \subseteq \lag^{-1}[\lag^{-k-1}[\W]] \subseteq \lag^{-k-2}[\W].  $$ So by the principle of induction, $\W \subseteq \lag^{-1}[\W] \subseteq \lag^{-2}[\W] \subseteq \lag^{-3}[\W]\subseteq\dots$ (ii). \\
			...
			\item 
			\begin{enumerate}
				\item Assume the hypothesis that $\lag^{k+1}[\W] = \lag^k[\W]$ for all $k \geq 0$. Also assume that $\lag^{k+j}[\W] = \lag^k[\W]$ for all non-negative $j$. The base case where $j=0$ is trivially true. The inductive case is also true since: $$ \lag^{k+j+1}[\W] =  \lag[\lag^{k+j}[\W]] = \lag[\lag^k[\W]] = \lag^{k+1}[\W] = \lag^k[\W]. $$ By the principle of induction, $\lag^{m}[\W] = \lag^k[\W]$ must hold for any $m\geq k\geq 0$ if $\lag^{k+1}[\W] = \lag^k[\W]$.\\
				
				\item Assume that hypothesis that $\lag^{-k}[\W] = \lag^{-(k+1)}[\W]$ for all $k \geq 0$. Also assume that $\lag^{-k}[\W] = \lag^{-(k+j)}[\W]$ for all non-negative $j$. The base case where $j=0$ is trivially true. In inductive case is also true since: 
				 \begin{align*}
				...
				Because $\dim(\lag^{-k}[\W]) \leq \dim(\V)$ for any $k\geq 0$, there exists $\lag^{-h}[\W]$, $h\geq 0$, be an invariant subspace in the chain with the highest dimension where $h$ is maximal. It immediately follows that $\dim(\lag^{-(h+1)}[\W]) = \dim(\lag^{-(h+1)}[\W])$ is maximal. But because $\lag^{-h}[\W] \subseteq \lag^{-(h+1)}[\W]$, $\lag^{-h}[\W] = \lag^{-(h+1)}[\W]$. As a result, $\lag^{-h}[\W] = \lag^{-j}[\W]$ for any $j\geq h$, which follows from (b). Therefore, the chain in part 1. stabilizes in the ``inclusion'' direction.  \\
				
				A similar argument can be given to show that the chain in part 1. also stabilizes in the ``inclusion by'' direction. Since $0 \leq \dim(\lag^{l}[\W])$ for any $l\geq 0$, there exists an $\lag^l[\W]$ in the chain with the lowest dimension but $l$ is minimal. It immediately follows that $\dim(\lag^{(l+1)}[\W]) = \dim(\lag^{l}[\W])$ is minimal. But because $\lag^{(l+1)}[\W] \subseteq \lag^{l}[\W]$, $\lag^{(l+1)}[\W] = \lag^{l}[\W]$. As a result, $\lag^{k}[\W] = \lag^{j}[\W]$ for any $k\leq l$, which follows from (a). Therefore, the chain in part 1. stabilizes in the ``inclusion by'' direction as well.\\
				
				For every \textit{proper} inclusion, there is a dimension loss. Revisiting $\lag^{-h}[\W]$, we know that $\dim(\lag^h[\W]) \leq \dim(\V)$. Consider the first proper inclusion: $$\dots \lag^{-(h+j)}[\W] \subsetneq \lag^{-(h+j-1)}[\W] \subseteq \dots \subseteq \lag^{-h}[\W]\subseteq \dots$$ 
				
				This implies $\dim(\lag^{-(h+j)}[\W]) < \dim(\lag^{-h}[\W]) \leq \dim(\V)$. Suppose $\dim(\lag^{-h}[\W]) = \dim(\V)$, i.e., maximal, then $$\dim(\lag^{-(h+j)}[\W]) \leq \dim(\V) - 1.$$ By assuming equality is met for and consider the next proper inclusion in the chain, a similar argument can show that the invariant subspace of interest will have at most $\dim(\V) - 2$ dimensions. It follows that at the $\dim(\V)^{\text{th}}$ proper inclusion, the invariant subspace has at most $\dim(\V) - \dim(\V) = 0$ dimensions; i.e. the subspace is $\{\mathbf{0}_\V\}$. At this point, no further proper inclusion is possible. Therefore, the chain in part 1. has at most $\dim(\V)$ proper inclusion.\\
			\end{enumerate}
			...
			\item	
			Let $\lag^{(k+1)}[\W] \subseteq \mathbf{M} \subseteq \lag^{k}[\W]$ be given for some integer $k$. $\mathbf{M}$ is a subspace. Observe that
			$$\lag[\mathbf{M}] \subseteq \lag[\lag^{k}[\W]] = \lag^{(k+1)}[\W] \subseteq \mathbf{M}. $$
			Therefore, $\mathbf{M}$ is invariant under $\lag$. 
