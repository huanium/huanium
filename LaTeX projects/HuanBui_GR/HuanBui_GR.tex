...
\end{exmp}
\begin{exmp}
	Find $U^{j'}_i$ for a rotation by an angle $\phi$ around the $z$-axis in Cartesian coordinates. Suppose $\vec{a} = \{a^i \}= (1,1,0)$. What is $\vec{a} = \{ a^{i'}\}$ after a rotation by $\phi$.
	\begin{align*}
	\text{insert figure here!!!}
	\end{align*}
	...
	But note that this is not true in general. As we have said before, coordinates don't generally form vectors, unless transformations are linear. 
\end{exmp}
