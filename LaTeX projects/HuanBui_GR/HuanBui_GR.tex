...
\end{align*}
Next, since $\vec{\lambda} = \lambda^{i'}\vec{e}_{i'} = \lambda^j\vec{e}_j = \lambda^jU^{i'}_j\vec{e}_{i'}$, we obtain the general coordinate transformation rule for contravariant vector components $\lambda_{i} \rightarrow \lambda^{i'}$ 
\begin{align*}
\boxed{\lambda^{i'} = U^{i'}_j\lambda^j}
\end{align*}
We can, of course, look at the Jacobian matrix representing the transformation $(S') \rightarrow (S)$:
\begin{align*}
[U^{j}_{i'}] = \left[ \frac{\partial u^j}{\partial u^{i'}}\right], 
\end{align*}
which is simply the inverse of $U^{i'}_j$. Therefore, it follows that:
\begin{align*}
[U^{k}_{i'}]
\end{align*}
