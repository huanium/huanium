...
options = optimoptions('fmincon','UseParallel',true, 'Algorithm','interior-point', 'Display','iter' ,...
    'ConstraintTolerance', 2e-5,'MaxFunctionEvaluations', 20000, 'MaxIterations', 1000,...
    'OptimalityTolerance', 2e-5, 'StepTolerance', 1e-4);
...
% clock starts
%%%%%%%%%%%%%%%%%%%
CZ_circuit_2N = CZ_circuit(2*N); % CZ entangling circuit
vec2 = zeros(N,1); % generate indices for state reduction (in overlap)
for k = 1:N
   vec2(N+1-k) = 2^(2*k-1);
end
