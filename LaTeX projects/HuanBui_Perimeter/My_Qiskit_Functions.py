...
M_simulaton = Aer.backends(name='qasm_simulator')[0]
...
def Wavefunction(obj, *args, **kwargs):
    ...
    if 'systems' in kwargs:
        ...
        if 'show_systems' in kwargs:
            ...
            if (len (systems)!= len(show_systems)):
                raise Exception('systems an show_systems need to be arrays of equal length')
    ...
    for i in np.arange(int(len(statevec))):
        ...
        if ((value.real!=0) or (value.imag!=0)):
            state = list(Binary2(int(i),int(qubits)))
            state_str = ''
            if (sys==True):
                k=0
