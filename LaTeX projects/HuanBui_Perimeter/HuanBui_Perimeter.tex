...
\begin{itemize}
	...
	\begin{proof}
		...
		&= \tr\lb T^{(2)} D_\had^{(2)} \f{\Id^{\otimes 2} + T^{(2)} }{d(d+1)} \rb \nn\\
		&= \f{1}{d(d+1)}\lb \tr\lb T^{(2)} \lb D_\had^{\otimes 2} \Id^{\otimes 2} + \D_\had^{\otimes 2} T^{(2)}  \rb \rb \rb\nn\\
		&= \f{1}{d(d+1)}\lb \tr\lb T^{(2)} \lb 2\Id^{\otimes 2}  \rb \rb \rb\nn\\
		&= \f{2}{d(d+1)}\tr\lb T^{\otimes 2} \rb\nn\\
		&= \f{2}{d(d+1)}d \nn\\
		&= \f{2}{d+1}
		\end{align}
		where we have used the fact that the original Hilbert space is $d$-dimensional. The swap operator $T^{(2)}$ by definition must be in $2d$ dimensions. But because it is a swap operator, it necessarily have $\tr T^{(2)} = 2d/2 =d$. So we're done. 
	\end{proof}
...
\begin{itemize}
	...
	\item \textbf{Use the fact that the $ZZ$ term is diagonal.} This will be the fastest way to get the Hamiltonian. 
\end{itemize}   
...
\begin{defn}[Unitarity]
	A $G$-module $V$ is unitary if it has an inner product and $\overline{\bra{\phi}g \ket{\phi}} = \bra{\phi}g^{-1}\ket{\phi}$.  
\end{defn}
...
\begin{defn}[Lie algebras]
	...
	\begin{itemize}
		\item  $X(\al\ket{\psi} + \beta\ket{\psi}) =\al X\ket{\psi} + \beta X\ket{\psi}$.
		
		\item $(\al X + \beta Y)\ket{\psi} = \al X\ket{\psi} + \beta Y\ket{\psi}$
		
		\item $[X,Y]\ket{\psi} = X(Y\ket{\psi}) - Y(X\ket{\psi})$.
	\end{itemize}
\end{defn}

In this case, $V$ is unitary if $\overline{\bra{\psi} X\ket{\phi}} = -\bra{\phi} X \ket{\psi}$.\\
