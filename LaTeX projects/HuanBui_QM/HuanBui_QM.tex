...
\begin{align}
\bra{\vec{k_3}\vec{k_4}}e^{-iH T}\ket{\vec{k_1} \vec{k_2}} = \bra{\vec{k_3}\vec{k_4}}e^{-i \int d^4x\, \lag(x) }\ket{\vec{k_1} \vec{k_2}} 
\end{align} 
for the following meson scattering process
\begin{align}
\vec{k_1} + \vec{k_2} \to \vec{k_3}+ \vec{k_4}
\end{align}
in order $\lambda$, with the anharmonic term $u(\phi) = \lambda \phi^4 / 4!$. By expanding first in $\lambda$, to first order in $\lambda$, this amplitude is 
\begin{align}
