...
\subsubsection{Parameterizing $M_0$.}



From the last section, we have seen that the expression for the scattering amplitude after Pauli-Villars regularization is
\begin{align}
\mathcal{M}  = \f{i\lambda^2}{32\pi^2} \int^1_0 d\alpha\,  \log\lp \f{\Lambda^2}{m^2  - \alpha(1 - \alpha)K^2 - i\epsilon} \rp 
\end{align}
However, be careful that this just an approximation where we have dropped a factor of $-1$. The more exact approximation comes from Eq. \eqref{exact}
\begin{align}
\mathcal{M}  = \f{i\lambda^2}{32\pi^2} \int^1_0 d\alpha\,\lb  \log\lp \f{\Lambda^2}{m^2  - \alpha(1 - \alpha)K^2 - i\epsilon} \rp -1 \rb 
\end{align}
For simplicity let's just assume that $m^2 \ll K^2$, so that we can ignore the $m^2$ term in the integrand. With this, we can rewrite $\mathcal{M}$ as
\begin{align}
\mathcal{M}  = \f{i\lambda^2}{32\pi^2} \int^1_0 d\alpha\,\lb  \log\lp \f{\Lambda^2}{ - \alpha(1 - \alpha)K^2 } \rp - 1\rb.
\end{align} 
We can evaluate this in Mathematica to get
\begin{lstlisting}
In[19]:= Integrate[Log[L^2/(-a (1 - a) K^2)], {a, 0, 1}]

Out[19]= 2 + Log[-(L^2/K^2)]
\end{lstlisting}
i.e.,
\begin{align}
\mathcal{M} = \f{i\lambda^2}{32\pi^2}\lb ? \rb
\end{align}
...
\subsubsection{Parameterization: a ``fix''}
\subsubsection{What is actually measured}
\subsubsection{The ``miracle''}
\subsubsection{Answers should be physical}
\subsubsection{Integrating over only slow modes}
