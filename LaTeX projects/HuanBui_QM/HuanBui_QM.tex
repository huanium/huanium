...
\begin{align}
i\p_t \psi = \beta m \psi - i \vec{\alpha} \cdot \grad{\psi} \equiv \ham_{\text{Dirac}}\psi.
\end{align}
...
\begin{align}
-\p_t^2 \psi &= \beta mi \p_t \psi + \vec{\alpha}\cdot \grad{\p_t \psi} \nonumber\\
&= \dots (\text{fill in here...})\nonumber\\
&= \beta^2 m^2 \psi  -im \lp \beta \vec{a} + \vec{a}\beta \rp\cdot \grad{\psi} - (\vec{a}\cdot \grad)^2 \psi \nonumber\\
&= m^2 \psi - \laplacian{\psi} \hspace{0.5cm}\text{KG equation}
\end{align}
...
\begin{align}
\begin{cases}
\beta^2 = \alpha_x^2 = \alpha_y^2 = \alpha_z^2\\
\beta \alpha_j + \alpha_j \beta = \alpha_j \alpha_k + \alpha_k \alpha_j = 0 \hspace{0.5cm}\forall j\neq k = x,y,z.
\end{cases}
\end{align}
...
From here, I'll just take Zee's words for it. Essentially, this leftover is not an easy integral to evaluate. Zee says that this one-loop $\phi^4$ amplitude comes out to be
\begin{align}
